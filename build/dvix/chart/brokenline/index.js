KISSY.add('dvix/chart/brokenline/', function (S, Dvix, Line, BrokenLine, Tools, xAxis, yAxis) {
    ...
    */
    ...
    var Brokenline = function (node) {
        ...
        this.data = null;
        this.dataRange = {
            //在上面的data池里面，当前可视尺寸内能现实的data范围，为以后的添加左右拖放放大缩小等扩展功能做准备
            start: 0,
            to: 0
        };
        this.fieldList = {
            //从上面的data获取得到的第一条数据的field字段列表
            title: '',
            index: 0
        };
        this.barWidth = 1;
        this.spaceWidth = 5;    //默认为5，实际上会在_calculateDataRange中计算出来
        //默认为5，实际上会在_calculateDataRange中计算出来
    ...
    Brokenline.prototype = {
        ...
        draw: function (data, options) {
            ...
            //开始绘图
            //初始化数据和配置
            self._config(data, options);    //从chart属性的data 里面获取yAxis xAxis的源data
            //从chart属性的data 里面获取yAxis xAxis的源data
            self._initData();    //得到了data后从Axis的data 计算 yAxis xAxis的layout相关
            //得到了data后从Axis的data 计算 yAxis xAxis的layout相关
            self._initLayout();    //计算barWidth spaceWidth
            //计算barWidth spaceWidth
            self._calculateDataRange();    //所有数据准备好后，终于开始绘图啦
            //所有数据准备好后，终于开始绘图啦
            self._startDraw();    //绘制结束，添加到舞台
            //绘制结束，添加到舞台
            self._drawEnd();
        },
        _config: function (data, options) {
            ...
            for (var i = 0, l = data[0].length; i < l; i++) {
                self.fieldList[data[0][i]] = { index: self._getIndexForField(data[0][i], data[0]) };
            }
            if (options.xAxis) {
                //如果有x轴
        ...
        _calculateDataRange: function () {
            ...
            self.spaceWidth = gwidth - self.barWidth;    //真正绘制之前 还要计算y轴的相关数据
            //真正绘制之前 还要计算y轴的相关数据
            self._yBlock = parseInt(gSpriteC.height / (self.yAxis.data.length - 1));    //v方向均分后还多余的部分px
            //v方向均分后还多余的部分px
        ...
        _graphsDraw: function () {
            ...
            var gSpriteC = graphs.sprite.context;    //画背景虚线
            //画背景虚线
            ...
            S.each(yAxis.fields, function (field, fi) {
                ...
                ;    //这个时候的数据要给xAxis保留一份
                //这个时候的数据要给xAxis保留一份
                self.xAxis.getxAxisPoints(pointList, groupWidth);    //设置原点
                //设置原点
