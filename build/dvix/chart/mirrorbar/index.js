...
;
        ...
        this.dataFrame = {
            ...
            yAxis: {
                ...
                //字段集合 对应this.data
                org: [],
                //二维 原始数据[[301,100,700],[201,145,88]]
                section: {
                    length: 2,
                    //用来标识外部数据传入几个数组
                    org: [],
                    //分段之后数据[1800,1200,600,0,600,1200,1800]
                    _org: [],
                    //分段之后数据 有负数[-1800,-1200,-600,0,600,1200,1800]
                    data: [],
                    //分段之后数据, 第一个和最后一个已删除[1200,600,0,600,1200]
                    _data: [],
                    //分段之后数据, 第一个和最后一个已删除 有负数[-1200,-600,0,600,1200]
                    maxs: [],
                    //最大两端的值,下端先push[1800,1800]
                    _maxs: [],
                    //最大两端的值,下端先push[-1800,1800]
                    _baseNumber: 0,
                    //基数 负数[-1200]
                ...
                //跨度,第一个值的绝对值 + 最后一个值的绝对值
                data: []    //坐标数据[{y:100,content:'100'},{y:200,content:'200'}] 与back.data相同但当config.yAxis.mode=2时    该值进行删减且重算，back.data不受影响
            },
            //坐标数据[{y:100,content:'100'},{y:200,content:'200'}] 与back.data相同但当config.yAxis.mode=2时    该值进行删减且重算，back.data不受影响
            graphs: {
                //图形
                data: [],
                //二维 数据集合等[[{x:0,y:-100},{}],[]]
    ...
    Mirrorbar.prototype = {
        init: function (node) {
            ...
            self.canvax = new Canvax({ el: self.element });
...
;
        ...
        _initData: function () {
            ...
            self.dataFrame.data = total    //已经处理成[o,o,o]   o={field:'val1',index:0,data:[1,2,3]}
;
        ...
        _startDraw: function () {
            ...
            self._yAxis.draw({ data: self.dataFrame.yAxis.data });
            ...
            self._back.draw({
                ...
                h: y,
                xAxis: { data: self.dataFrame.yAxis.data }
            });
            self._back.setX(x), self._back.setY(y)    //self._trimGraphs()
;
            //self._trimGraphs()
            self._graphs.draw({
        ...
        _getSection: function () {
            //获取处理后的y轴数据(原始包含两端最大值的数组)
            var self = this;
            var section = [];
            var _section = [];
            var arr = self.dataFrame.yAxis.org;
            var maxPart = parseInt((self.config.dataSection.maxPart - 1) / 2)    // console.log(DataSection.section([0,0,0,0,0,0], maxPart, {mode:1}))
;
        ...
        _trimYAxis: function () {
            ...
            var contents = self.dataFrame.yAxis.section.data    // var baseNumber = 
;
            // var baseNumber = 
            var tmpData = [];
            for (var a = 0, al = arr.length; a < al; a++) {
                var y = -(arr[a] - self._baseNumber) / (max - self._baseNumber) * self._yGraphsHeight    // var y = - (arr[a] - self._baseNumber) / (max - self._baseNumber) * self._yGraphsHeight
        ...
        _trimGraphs: function () {
            ...
            for (var a = 0, al = arr.length; a < al; a++) {
                for (var b = 0, bl = arr[a].length; b < bl; b++) {
                    ...
                    tmpData[a][b] = {
                        'value': arr[a][b],
                        'x': b / (maxXAxis - 1) * self._xGraphsWidth,
                        'y': y
                    };
                }
            }
