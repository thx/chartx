KISSY.add('dvix/index', function (S) {
    window.Site = {
                ...
                S.getScript(preFiles.shift(), {
                    success: function () {
                        if (preFiles.length > 0) {
                            ...
                            preIsReady = true;    //如果临时任务队列中有任务。执行任务
                            //如果临时任务队列中有任务。执行任务
                            if (currTaskList.length > 0) {
                                (function () {
                                    ...
                                    useConstructor(task.chart, function (chart) {
                                        task.callback(chart);    //然后看有没有下一个任务
                                        //然后看有没有下一个任务
                                        if (currTaskList.length > 0) {
                                            fun();
                                        }
    ...
    function useConstructor(chart, callback) {
        S.use('dvix/chart/' + chart.name.toLowerCase() + '/', function (S, Chart) {
