...
;
        ...
        var initMax = max;
        max *= scale    // var min = arrayMin(arr);
;
        // var min = arrayMin(arr);
        var min = Math.min.apply(null, arr);
        if (min == max) {
            if (max >= 0) {
                min = 0    // min= Math.round(max/2);
;
            } else
...
;
        ...
        var pos, lastPos, roundedMin = correctFloat(Math.floor(min / tickInterval) * tickInterval), roundedMax = correctFloat(Math.ceil(max / tickInterval) * tickInterval), tickPositions = [];    // Populate the intermediate values
