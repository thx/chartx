...
(function (S, undefined) {
    var MIX_CIRCULAR_DETECTION = '__MIX_CIRCULAR', STAMP_MARKER = '__~ks_stamped', host = this, TRUE = true, EMPTY = '', ObjectCreate = Object.create,
        // error in native ie678, not in simulated ie9
        hasEnumBug = !{ toString: 1 }['propertyIsEnumerable']('toString'), enumProperties = [
            'constructor',
            'hasOwnProperty',
            'isPrototypeOf',
    ...
    mix(S, {
        ...
        augment: function (r, var_args) {
            var args = S.makeArray(arguments), len = args.length - 2, i = 1, ov = args[len], wl = args[len + 1];
            if (!S.isArray(wl)) {
                ov = wl;
                wl = undefined;
                len++;
        ...
        extend: function (r, s, px, sx) {
            ...
            var sp = s.prototype, rp;    // add prototype chain
            // add prototype chain
...
(function (S, undefined) {
    ...
    S.mix(S, {
        ...
        each: function (object, fn, context) {
            if (object) {
                var key, val, keys, i = 0, length = object && object.length, isObj = length === undefined || S.type(object) === 'function';
        ...
        makeArray: function (o) {
            ...
            if (lengthType != 'number' || // form.elements in ie78 has nodeName 'form'
                // then caution select
                // o.nodeName
                // window
                o.alert || oType == 'string' || // https://github.com/ariya/phantomjs/issues/11478
...
(function (S, undefined) {
    ...
    var SEP = '&', EMPTY = '', EQ = '=', TRUE = true,
        ...
        htmlEntities = {
            ...
            '&quot;': '"',
            '&#x27;': '\''
        }, reverseEntities = {}, escapeReg, unEscapeReg,
        // - # $ ^ * ( ) + [ ] { } | \ , . ?
        escapeRegExp = /[\-#$\^*()+\[\]{}|\\,.?\s]/g;
    (function () {
...
(function (S, undefined) {
    ...
    function cloneInternal(input, f, memory) {
        ...
        if (input[CLONE_MARKER]) {
            ...
            else // ImageData , File, Blob , FileList .. etc
...
(function (S, undefined) {
    // [[Class]] -> type pairs
    var class2type = {}, FALSE = false, OP = Object.prototype, toString = OP.toString;
...
(function (S, undefined) {
    ...
    function promiseWhen(promise, fulfilled, rejected) {
        ...
        var v = promise[PROMISE_VALUE], pendings = promise[PROMISE_PENDINGS];    // unresolved
        ...
        else // rejected or nested promise
        if (isPromise(v)) {
     ...
     */
    function Promise(v) {
        var self = this;    // maybe internal value is also a promise
    ...
    function when(value, fulfilled, rejected) {
        ...
        function _fulfilled(value) {
            try {
                return fulfilled ? fulfilled(value) : // propagate
                value;
            } catch (e) {
    ...
    function isResolved(obj) {
        ...
        return !isRejected(obj) && isPromise(obj) && // self is resolved
        obj[PROMISE_PENDINGS] === undefined && // value is a resolved promise or value is immediate value
        // immediate value
    ...
    var Path = {
         ...
         */
            normalize: function (path) {
                var absolute = path.charAt(0) == '/', trailingSlash = path.slice(-1) == '/';
 ...
 */
(function (S, undefined) {
    var reDisallowedInSchemeOrUserInfo = /[#\/\?@]/g, reDisallowedInPathName = /[#\?]/g,
        // ?? combo of taobao
        reDisallowedInQuery = /[#@]/g, reDisallowedInFragment = /#/g, URI_SPLIT_REG = new RegExp('^' + /*
    ...
    Uri.prototype = {
        ...
        resolve: function (relativeUri) {
            ...
            var self = this, override = 0, lastSlashIndex, order = [
                    'scheme',
                    'userInfo',
                    'hostname',
                    'port',
        ...
        isSameOriginAs: function (other) {
            var self = this;    // port and hostname has to be same
...
(function (S, undefined) {
    ...
    function getDescriptorFromUserAgent(ua) {
        ...
        if (s.length > 0) {
            ...
            if ((m = ua.match(/AppleWebKit\/([\d.]*)/)) && m[1]) {
                ...
                else // Other WebKit Mobile Browsers
    ...
    var UA = KISSY.UA = getDescriptorFromUserAgent(ua);    // nodejs
    // nodejs
 ...
 */
(function (S) {
    var Env = S.Env, win = Env.host, UA = S.UA,
        // nodejs
        doc = win.document || {},
        // phantomjs issue: http://code.google.com/p/phantomjs/issues/detail?id=375
        isTouchSupported = 'ontouchstart' in doc && !UA.phantomjs, documentMode = doc.documentMode, ie = documentMode || UA.ie, isNativeJSONSupported = (Env.nodejs && typeof global === 'object' ? global : win).JSON;    // ie 8.0.7600.16315@win7 json bug!
 ...
 */
(function (S) {
    var Loader = S.Loader, Path = S.Path, host = S.Env.host, startsWith = S.startsWith, data = Loader.Status, ATTACHED = data.ATTACHED, LOADED = data.LOADED, ERROR = data.ERROR,
         ...
         * @private
         */
        Utils = Loader.Utils = {}, doc = host.document;    // http://wiki.commonjs.org/wiki/Packages/Mappings/A
                                                           // 如果模块名以 / 结尾，自动加 index
    ...
    S.mix(Utils, {
        ...
        attachModRecursively: function (modName, runtime, stack, errorList) {
            ...
            if (Utils.attachModsRecursively(m.getNormalizedRequires(), runtime, stack, errorList)) {
        ...
        registerModule: function (runtime, name, fn, config) {
            ...
            mod = mods[name];    // 注意：通过 S.add(name[, fn[, config]]) 注册的代码，无论是页面中的代码，
            ...
            S.mix(mod, config);    // S.log(name + ' is loaded', 'info');
        },
        ...
        getMappedPath: function (runtime, path, rules) {
            var mappedRules = rules || runtime.Config.mappedRules || [], i, m, rule;
...
(function (S) {
    ...
    S.augment(Package, {
        ...
        getPrefixUriForCombo: function () {
            ...
            return self.getBase() + (packageName && !self.isIgnorePackageNameInUri() ? packageName + '/' : '');
        },
    ...
    S.augment(Module, {
        ...
        getFullPath: function () {
            ...
            if (!self.fullpath) {
                ...
                path = self.getPath();    // #262
                // #262
                if (packageInfo.isIgnorePackageNameInUri() && // native mod does not allow ignore package name
        ...
        getPackage: function () {
            var self = this;
            return self.packageInfo || (self.packageInfo = getPackage(self.runtime, self.name));
        },
        /**
        ...
        getNormalizedRequires: function () {
            ...
            if (!requires || requires.length == 0) {
                return requires || [];
            } else if ((normalizedRequires = self.normalizedRequires) && // 事先声明的依赖不能当做 loaded 状态下真正的依赖
 ...
 */
(function (S) {
    var CSS_POLL_INTERVAL = 30, UA = S.UA, logger = {
            debug: function (str) {
    ...
    function isCssLoaded(node, url) {
        ...
        if (UA.webkit) {
            ...
            try {
                ...
                var exName = ex.name;
                logger.debug('css poll exception: ' + exName + ' ' + ex.code + ' ' + url);    // http://www.w3.org/TR/dom/#dom-domexception-code
                if (// http://www.w3.org/TR/dom/#dom-domexception-code
...
(function (S) {
    ...
    S.getScript = function (url, success, charset) {
        ...
        var useNative = 'onload' in node;    // onload for webkit 535.23  Firefox 9.0
                                             // https://bugs.webkit.org/show_activity.cgi?id=38995
                                             // https://bugzilla.mozilla.org/show_bug.cgi?id=185236
                                             // https://developer.mozilla.org/en/HTML/Element/link#Stylesheet_load_events
        ...
        else // old chrome/firefox for css
        if (css) {
...
(function (S, undefined) {
    ...
    configFns.packages = function (cfgs) {
        var name, Config = this.Config, ps = Config.packages = Config.packages || {};
        if (cfgs) {
            S.each(cfgs, function (cfg, key) {
                // 兼容数组方式
                name = cfg.name || key;    // 兼容 path
                // 兼容 path
...
(function (S, undefined) {
    var Loader, Status, Utils, UA,
        ...
        startLoadTime = 0,
        // ie6,7,8开始载入脚本对应的模块名
        startLoadModName, LOADING, LOADED, ERROR, ATTACHED;
    Loader = S.Loader;
    ...
    S.augment(SimpleLoader, {
        ...
        loadModule: function (modName) {
            ...
            if (status === LOADED) {
                ...
                isWait = waitingModules.contains(modName);    // prevent duplicate listen for this use
                                                              //  use('a,a') or
                ...
                mod.addCallback(function () {
                    ...
                    self.loadModRequires(mod);
                    waitingModules.remove(modName);    // notify current loader instance
                    // notify current loader instance
                    waitingModules.notifyAll();
                });
                waitingModules.add(modName);    // in case parallel use (more than one use)
        ...
        fetchModule: function (mod) {
            var self = this, runtime = self.runtime, modName = mod.getName(), charset = mod.getCharset(), url = mod.getFullPath(), ie = UA.ie, isCss = mod.getType() == 'css';
            mod.status = LOADING;
            if (ie && !isCss) {
            ...
            S.getScript(url, {
                attrs: ie ? { 'data-mod-name': modName } : undefined,
    ...
    SimpleLoader.add = function (name, fn, config, runtime) {
        if (typeof name === 'function') {
            ...
            if (UA.ie) {
                ...
                name = findModuleNameByInteractive();    // S.log('ie get modName by interactive: ' + name);
...
(function (S) {
    function loadScripts(rss, callback, charset) {
        var count = rss && rss.length, errorList = [], successList = [];
        function complete() {
            if (!--count) {
    ...
    var Loader = S.Loader, Status = Loader.Status, Utils = Loader.Utils, LOADING = Status.LOADING, LOADED = Status.LOADED, ERROR = Status.ERROR, groupTag = S.now(), ATTACHED = Status.ATTACHED;
    ...
    S.augment(ComboLoader, {
         ...
         */
        use: function (normalizedModNames) {
            var self = this, allModNames, comboUrls, runtime = self.runtime;
            allModNames = S.keys(self.calculate(normalizedModNames));
            Utils.createModulesInfo(runtime, allModNames);
            comboUrls = self.getComboUrls(allModNames);    // load css first to avoid page blink
                ...
                loadScripts(cssOne, function (success, error) {
                    ...
                    S.each(success, function (one) {
                        S.each(one.mods, function (mod) {
                            Utils.registerModule(runtime, mod.getName(), S.noop);    // notify all loader instance
                            // notify all loader instance
                            mod.notifyAll();
                        });
                    });
                    S.each(error, function (one) {
                        S.each(one.mods, function (mod) {
                            ...
                            S.log(msg, 'error');
                            mod.status = ERROR;    // notify all loader instance
                            // notify all loader instance
                            mod.notifyAll();
                        });
                    });
                ...
                if (modStatus != LOADED && modStatus != ATTACHED) {
                    if (!waitingModules.contains(m)) {
                        ...
                        mod.addCallback(function (mod) {
                            waitingModules.remove(mod.getName());    // notify current loader instance
                            // notify current loader instance
                            waitingModules.notifyAll();
                        });
        ...
        getComboMods: function (modNames, comboPrefixes) {
            ...
            for (; i < l; ++i) {
                ...
                var comboName = packageName;    // whether group packages can be combined (except default package and non-combo modules)
                // whether group packages can be combined (except default package and non-combo modules)
                if ((mod.canBeCombined = packageInfo.isCombine() && S.startsWith(fullpath, packagePath)) && group) {
        ...
        getComboUrls: function (modNames) {
            ...
            var comboPrefixes = {};    // {type, {comboName, [modInfo]}}}
            // {type, {comboName, [modInfo]}}}
            var comboMods = this.getComboMods(modNames, comboPrefixes);    // {type, {comboName, [url]}}}
            // {type, {comboName, [url]}}}
            var comboRes = {};    // generate combo urls
            // generate combo urls
            for (var type in comboMods) {
                ...
                for (var comboName in comboMods[type]) {
                    ...
                    for (var i = 0; i < mods.length; i++) {
                        ...
                        res.mods.push(currentMod);    // map individual module
                        ...
                        if (currentComboUrls.length > maxFileNum || l + currentComboUrls.join(comboSep).length + suffixLength > maxUrlLength) {
...
(function (S, undefined) {
    ...
    S.mix(S, {
        ...
        require: function (name) {
            return Utils.getModules(S, Utils.normalizeModNamesWithAlias(S, [name]))[1];
        }
    });
    ...
    function getBaseInfoFromOneScript(script) {
        ...
        var parts, base, index = src.indexOf(comboPrefix);    // no combo
        // no combo
        if (index == -1) {
            base = src.replace(baseReg, '$1');
        } else {
            base = src.substring(0, index);    // a.tbcdn.cn??y.js, ie does not insert / after host
        ...
        return S.mix({ base: base }, baseInfo);
    }    /**
 ...
 */
(function (S, undefined) {
    var win = S.Env.host, UA = S.UA, doc = win['document'], docElem = doc && doc.documentElement, location = win.location, EMPTY = '', readyDefer = new S.Defer(), readyPromise = readyDefer.promise,
        // The number of poll times.
        POLL_RETIRES = 500,
        // The poll interval in milliseconds.
        POLL_INTERVAL = 40,
        // #id or id
        RE_ID_STR = /^#?([\w-]+)$/, RE_NOT_WHITESPACE = /\S/, standardEventModel = !!(doc && doc.addEventListener), DOM_READY_EVENT = 'DOMContentLoaded', READY_STATE_CHANGE_EVENT = 'readystatechange', LOAD_EVENT = 'load', COMPLETE = 'complete', addEventListener = standardEventModel ? function (el, type, fn) {
    ...
    S.mix(S, {
        ...
        available: function (id, fn) {
            ...
            var retryCount = 1, node, timer = S.later(function () {
                    if ((node = doc.getElementById(id)) && (fn(node) || 1) || ++retryCount > POLL_RETIRES) {
                        timer.cancel();
                    }
                }, POLL_INTERVAL, true);
        }
...
(function (config, Features, UA) {
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'base': { requires: ['event/custom'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'button': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'color': { requires: ['base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'combobox': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'component/plugin/resize': { requires: ['resizable'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'datalazyload': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'dom/ie': { requires: ['dom/base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'editor': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'event/custom': { requires: ['event/base'] } });
    config({
        'event/dom': {
            'alias': [
                'event/dom/base',
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'event/dom/focusin': { requires: ['event/dom/base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'event/dom/shake': { requires: ['event/dom/base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'event/dom/touch': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'json': { requires: [KISSY.Features.isNativeJSONSupported() ? '' : 'json/json2'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({ 'kison': { requires: ['base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'menu': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'rich-base': { requires: ['base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({ 'separator': { requires: ['component/base'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'split-button': {
            requires: [
    ...
    /*Generated by KISSY Module Compiler*/
    config({ 'stylesheet': { requires: ['dom'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'swf': {
            requires: [
    ...
    config({ xtemplate: { alias: ['xtemplate/facade'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({ 'xtemplate/compiler': { requires: ['xtemplate/runtime'] } });    /*Generated by KISSY Module Compiler*/
    /*Generated by KISSY Module Compiler*/
    config({
        'xtemplate/facade': {
            requires: [
...
KISSY.add('dom/base/api', function (S) {
    var WINDOW = S.Env.host, UA = S.UA, RE_NUM = /[\-+]?(?:\d*\.|)\d+(?:[eE][\-+]?\d+|)/.source,
        /**
    ...
    var DOM = {
         ...
         */
            getWindow: function (elem) {
                ...
                return 'scrollTo' in elem && elem['document'] ? elem : elem.nodeType == NodeType.DOCUMENT_NODE ? elem.defaultView || elem.parentWindow : false;
            },
    ...
    var doc = S.Env.host.document, NodeType = DOM.NodeType, docElement = doc && doc.documentElement, TEXT = docElement && docElement.textContent === undefined ? 'innerText' : 'textContent', EMPTY = '', nodeName = DOM.nodeName, R_BOOLEAN = /^(?:autofocus|autoplay|async|checked|controls|defer|disabled|hidden|loop|multiple|open|readonly|required|scoped|selected)$/i, R_FOCUSABLE = /^(?:button|input|object|select|textarea)$/i, R_CLICKABLE = /^a(?:rea)?$/i, R_INVALID_CHAR = /:|^on/, R_RETURN = /\r/g, attrFix = {}, attrFn = {
        ...
        attrNodeHook = {}, valHooks = {
            select: {
                ...
                get: function (elem) {
                    var index = elem.selectedIndex, options = elem.options, ret, i, len, one = elem.type === 'select-one';    // Nothing was selected
         ...
         */
    {
        ...
        attr: function (selector, name, val, pass) {
            ...
            if (val === undefined) {
                if (el && el.nodeType === NodeType.ELEMENT_NODE) {
                    ...
                    ret = el.getAttribute(name);    // standard browser non-existing attribute return null
        ...
        val: function (selector, value) {
            ...
            for (i = els.length - 1; i >= 0; i--) {
                ...
                hook = valHooks[nodeName(elem)] || valHooks[elem.type];    // If set returns undefined, fall back to normal setting
        ...
        text: function (selector, val) {
            ...
            if (val === undefined) {
                ...
                el = DOM.get(selector);    // only gets value on supported nodes
...
KISSY.add('dom/base/class', function (S, DOM, undefined) {
    var SPACE = ' ', NodeType = DOM.NodeType, RE_SPLIT = /[\.\s]\s*\.?/, RE_CLASS = /[\n\t]/g;
    ...
    S.mix(DOM, {
        ...
        hasClass: function (selector, className) {
            return batch(selector, className, function (elem, classNames, cl) {
                var elemClass = elem.className, className, j, ret;
                if (elemClass) {
        ...
        addClass: function (selector, className) {
            batch(selector, className, function (elem, classNames, cl) {
                var elemClass = elem.className, normClassName, setClass, j;
                if (!elemClass) {
        ...
        removeClass: function (selector, className) {
            batch(selector, className, function (elem, classNames, cl) {
                var elemClass = elem.className, className, j, needle;
                if (elemClass) {
                    if (!cl) {
                        elem.className = '';
                    } else {
    ...
    function batch(selector, value, fn, resultIsBool) {
        ...
        var elems = DOM.query(selector), len = elems.length, tmp = value.split(RE_SPLIT), elem, ret, classNames = [], t, i;
...
KISSY.add('dom/base/create', function (S, DOM, undefined) {
    var doc = S.Env.host.document, NodeType = DOM.NodeType, UA = S.UA, ie = UA['ie'], DIV = 'div', PARENT_NODE = 'parentNode', DEFAULT_DIV = doc && doc.createElement(DIV), R_XHTML_TAG = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/gi, RE_TAG = /<([\w:]+)/, R_LEADING_WHITESPACE = /^\s+/, lostLeadingWhitespace = ie && ie < 9, R_HTML = /<|&#?\w+;/, supportOuterHTML = doc && 'outerHTML' in doc.documentElement, RE_SIMPLE_TAG = /^<(\w+)\s*\/?>(?:<\/\1>)?$/;    // help compression
    // help compression
    ...
    function defaultCreator(html, ownerDoc) {
        var frag = ownerDoc && ownerDoc != doc ? ownerDoc.createElement(DIV) : DEFAULT_DIV;    // html 为 <style></style> 时不行，必须有其他元素？
    ...
    S.mix(DOM, {
        ...
        html: function (selector, htmlString, loadScripts, callback) {
            ...
            if (htmlString === undefined) {
                ...
                // setter
                {
                    ...
                    // faster
                    // fix #103,some html element can not be set through innerHTML
                    if (!htmlString.match(/<(?:script|style|link)/i) && (!lostLeadingWhitespace || !htmlString.match(R_LEADING_WHITESPACE)) && !creatorsMap[(htmlString.match(RE_TAG) || [
                            '',
                            ''
                        ])[1].toLowerCase()]) {
                        try {
        ...
        outerHTML: function (selector, htmlString, loadScripts) {
            var els = DOM.query(selector), holder, i, valNode, ownerDoc, length = els.length, el = els[0];
            if (!el) {
                return;
            }    // getter
            // getter
            if (htmlString === undefined) {
                if (supportOuterHTML) {
                    ...
                    holder = ownerDoc && ownerDoc != doc ? ownerDoc.createElement(DIV) : DEFAULT_DIV;
                    holder.innerHTML = '';
        ...
        remove: function (selector, keepData) {
            var el, parent, els = DOM.query(selector), elChildren, i;
            for (i = els.length - 1; i >= 0; i--) {
                el = els[i];
    ...
    function cloneWithDataAndEvent(src, dest) {
        var Event = S.require('event/dom'), srcData, d;
        ...
        if (Event) {
            ...
            Event._DOMUtils.removeData(dest);    // attach src 's event data and dom attached listener to dest
...
KISSY.add('dom/base/data', function (S, DOM, undefined) {
    var win = S.Env.host, EXPANDO = '__ks_data_' + S.now(),
            ...
            removeData: function (elem, name) {
                ...
                if (name !== undefined) {
                    ...
                    try {
                        ...
                        elem[EXPANDO] = undefined;    //S.log('delete expando error : ');
                                                      //S.log(e);
                    }
...
KISSY.add('dom/base/offset', function (S, DOM, undefined) {
    var win = S.Env.host, doc = win.document, NodeType = DOM.NodeType, docElem = doc && doc.documentElement, getWin = DOM.getWindow, CSS1Compat = 'CSS1Compat', compatMode = 'compatMode', MAX = Math.max, myParseFloat = parseFloat, POSITION = 'position', RELATIVE = 'relative', DOCUMENT = 'document', BODY = 'body', DOC_ELEMENT = 'documentElement', OWNER_DOCUMENT = 'ownerDocument', VIEWPORT = 'viewport', SCROLL = 'scroll', CLIENT = 'client', LEFT = 'left', TOP = 'top', isNumber = S.isNumber, SCROLL_LEFT = SCROLL + 'Left', SCROLL_TOP = SCROLL + 'Top';
    S.mix(DOM, {
        /**
         * @override KISSY.DOM
         * @class
        ...
        scrollIntoView: function (selector, container, alignWithTop, allowHorizontalScroll) {
            ...
            var isWin = !!getWin(container), elemOffset = DOM.offset(elem), eh = DOM.outerHeight(elem), ew = DOM.outerWidth(elem), containerOffset, ch, cw, containerScroll, diffTop, diffBottom, win, winScroll, ww, wh;
            if (isWin) {
                win = container;
                wh = DOM.height(win);
                ww = DOM.width(win);
                ...
                // 注意边框 , offset 是边框到根节点
                diffTop = {
                    left: elemOffset[LEFT] - (containerOffset[LEFT] + (myParseFloat(DOM.css(container, 'borderLeftWidth')) || 0)),
                    top: elemOffset[TOP] - (containerOffset[TOP] + (myParseFloat(DOM.css(container, 'borderTopWidth')) || 0))
                };
                diffBottom = {
                    left: elemOffset[LEFT] + ew - (containerOffset[LEFT] + cw + (myParseFloat(DOM.css(container, 'borderRightWidth')) || 0)),
                    top: elemOffset[TOP] + eh - (containerOffset[TOP] + ch + (myParseFloat(DOM.css(container, 'borderBottomWidth')) || 0))
                };
            }
    ...
    S.each([
        ...
        DOM[method] = function (elem, v) {
            ...
            elem = DOM.get(elem);
            var ret, left, top, w = getWin(elem), d;
            if (w) {
                if (v !== undefined) {
    ...
    S.each([
        ...
        DOM['doc' + name] = function (refWin) {
            ...
            var w = getWin(refWin), d = w[DOCUMENT];
        ...
        DOM[VIEWPORT + name] = function (refWin) {
            refWin = DOM.get(refWin);
            var prop = CLIENT + name, win = getWin(refWin), doc = win[DOCUMENT], body = doc[BODY], documentElement = doc[DOC_ELEMENT], documentElementProp = documentElement[prop];    // 标准模式取 documentElement
                                                                                                                                                                                       // backcompat 取 body
    ...
    function getPageOffset(el) {
        var pos = getClientPosition(el), w = getWin(el[OWNER_DOCUMENT]);
    ...
    function getOffset(el, relativeWin) {
        var position = {
            ...
            currentWin = getWin(el[OWNER_DOCUMENT]), offset, currentEl = el;
...
KISSY.add('dom/base/selector', function (S, DOM, undefined) {
    var doc = S.Env.host.document, NodeType = DOM.NodeType, filter = S.filter, require = function (selector) {
            return S.require(selector);
        }, isArray = S.isArray, makeArray = S.makeArray, isNodeList = DOM._isNodeList, getNodeName = DOM.nodeName, push = Array.prototype.push, SPACE = ' ', COMMA = ',', trim = S.trim, RE_ID = /^#[\w-]+$/, RE_QUERY = /^(?:#([\w-]+))?\s*([\w-]+|\*)?\.?([\w-]+)?$/;
    ...
    function query(selector, context) {
        var ret, i, simpleContext, isSelectorString = typeof selector == 'string', contexts = context === undefined && (simpleContext = 1) ? [doc] : query(context);    // 常见的空
        // 常见的空
        if (!selector) {
            ret = [];
        }    // 常见的选择器
        ...
        // DOM.query('#x')
        if (isSelectorString) {
            selector = trim(selector);    // shortcut
            // shortcut
        ...
        if (simpleContext) {
            ...
            else // 2.KISSY NodeList 特殊点直接返回，提高性能
        ...
        if (!ret) {
            ...
            if (selector) {
                ...
                //必要时去重排序
                if (ret.length > 1 && // multiple contexts
                    (contexts.length > 1 || isSelectorString && // multiple selector
    ...
    function queryByContexts(selector, context) {
        var ret = [], isSelectorString = typeof selector == 'string';
        if (isSelectorString && selector.match(RE_QUERY) || !isSelectorString) {
            // 简单选择器自己处理
            ret = queryBySimple(selector, context);
        }    // 如果选择器有, 分开递归一部分一部分来
        else // 如果选择器有, 分开递归一部分一部分来
        if (isSelectorString && // #255
            // [data-key='a,b']
            selector.replace(/"(?:(?:\\.)|[^"])*"/g, '').replace(/'(?:(?:\\.)|[^'])*'/g, '').indexOf(COMMA) > -1) {
    ...
    function queryBySelectors(selector, context) {
        var ret = [], i, selectors = selector.split(/\s*,\s*/);
    ...
    function quickFindBySelectorStr(selector, context) {
        var ret, t, match, id, tag, cls;    // selector 为 #id 是最常见的情况，特殊优化处理
        // selector 为 #id 是最常见的情况，特殊优化处理
        if (RE_ID.test(selector)) {
            t = getElementById(selector.slice(1), context);    // #id 无效时，返回空数组
            // #id 无效时，返回空数组
            ret = t ? [t] : [];
            ...
            // selector 为支持列表中的其它 6 种
            {
                ...
                if (match) {
                    ...
                    id = match[1];
                    tag = match[2];
                    cls = match[3];    // 空白前只能有 id ，取出来作为 context
    ...
    function queryBySimple(selector, context) {
        ...
        else // 传入的 selector 是 NodeList 或已是 Array
        ...
        else // 传入的 selector 是 HTMLNode 查看约束
        // 否则 window/document，原样返回
    ...
    function getElementById(id, context) {
        var contextIsDocument = context.nodeType == NodeType.DOCUMENT_NODE, doc = contextIsDocument ? context : context.ownerDocument;
         ...
         */
    {
        ...
        _getElementById: function (id, context, doc, contextIsDocument) {
            var el = doc.getElementById(id);    // ie confuse name with id
            ...
            if (!el && !contextIsDocument && !DOM._contains(doc, context) || el && elId != id) {
        ...
        filter: function (selector, filter, context) {
            var elems = query(selector, context), sizzle = require('sizzle'), match, tag, id, cls, ret = [];    // 默认仅支持最简单的 tag.cls 或 #id 形式
            // 默认仅支持最简单的 tag.cls 或 #id 形式
            if (typeof filter == 'string' && (filter = trim(filter)) && (match = RE_QUERY.exec(filter))) {
                id = match[1];
                tag = match[2];
                cls = match[3];
                if (!id) {
...
KISSY.add('dom/base/style', function (S, DOM, undefined) {
    var WINDOW = S.Env.host, UA = S.UA, getNodeName = DOM.nodeName, doc = WINDOW.document, STYLE = 'style', FLOAT = 'float', RE_MARGIN = /^margin/, WIDTH = 'width', HEIGHT = 'height', AUTO = 'auto', DISPLAY = 'display', OLD_DISPLAY = DISPLAY + S.now(), NONE = 'none', myParseInt = parseInt, cssNumber = {
            'fillOpacity': 1,
            'fontWeight': 1,
            'lineHeight': 1,
            'opacity': 1,
            ...
            'zIndex': 1,
            'zoom': 1
        }, rmsPrefix = /^-ms-/, RE_DASH = /-([a-z])/gi, CAMEL_CASE_FN = function (all, letter) {
            return letter.toUpperCase();
        },
        // 考虑 ie9 ...
        R_UPPER = /([A-Z]|^ms)/g, EMPTY = '', DEFAULT_UNIT = 'px', CUSTOM_STYLES = {}, cssProps = {}, defaultDisplay = {};
         ...
         */
    {
        ...
        _getComputedStyle: function (elem, name) {
            ...
            name = name.replace(R_UPPER, '-$1').toLowerCase();    // https://github.com/kissyteam/kissy/issues/61
        ...
        css: function (selector, name, val) {
            ...
            name = camelCase(name);
            hook = CUSTOM_STYLES[name];    // getter
            // getter
            if (val === undefined) {
                // supports css selector/Node/NodeList
                ret = '';
        ...
        addStyleSheet: function (refWin, cssText, id) {
            ...
            refWin = DOM.get(refWin);
            var win = DOM.getWindow(refWin), doc = win.document, elem;
        ...
        unselectable: function (selector) {
            var _els = DOM.query(selector), elem, j, e, i = 0, excludes, els;
    ...
    S.each([
        ...
        CUSTOM_STYLES[name] = {
            get: function (elem, computed) {
                var val, offset;
                if (computed) {
                    val = DOM._getComputedStyle(elem, name);    // 1. 当没有设置 style.left 时，getComputedStyle 在不同浏览器下，返回值不同
                    ...
                    if (val === AUTO) {
                        val = 0;
                        if (S.inArray(DOM.css(elem, 'position'), [
                                'absolute',
                                'fixed'
                            ])) {
                            offset = elem[name === 'left' ? 'offsetLeft' : 'offsetTop'];    // old-ie 下，elem.offsetLeft 包含 offsetParent 的 border 宽度，需要减掉
                            ...
                            if (UA.ie && (doc['documentMode'] || 0) < 9 || UA['opera']) {
                                ...
                                offset -= elem.offsetParent && elem.offsetParent['client' + (name == 'left' ? 'Left' : 'Top')] || 0;
                            }
    ...
    function swap(elem, options, callback) {
        var old = {}, name;    // Remember the old values, and insert the new ones
    ...
    function style(elem, name, val) {
        ...
        name = cssProps[name] || name;    // setter
        // setter
        if (val !== undefined) {
            // normalize unset
            ...
            else // number values may need a unit
...
KISSY.add('dom/ie/attr', function (S, DOM) {
    var attrHooks = DOM._attrHooks, attrNodeHook = DOM._attrNodeHook, NodeType = DOM.NodeType, valHooks = DOM._valHooks, propFix = DOM._propFix, HREF = 'href', hrefFix, IE_VERSION = document.documentMode || S.UA.ie;
    if (IE_VERSION < 8) {
        ...
        // ie6,7 不区分 attribute 与 property
        S.mix(DOM._attrFix, propFix);    // http://fluidproject.org/blog/2008/01/09/getting-setting-and-removing-tabindex-values-with-javascript/
        // http://fluidproject.org/blog/2008/01/09/getting-setting-and-removing-tabindex-values-with-javascript/
...
KISSY.add('dom/ie/create', function (S, DOM) {
    var IE_VERSION = document.documentMode || S.UA.ie;    // wierd ie cloneNode fix from jq
    ...
    DOM._fixCloneAttributes = function (src, dest) {
        ...
        var nodeName = dest.nodeName.toLowerCase(), srcChildren = src.childNodes;    // IE6-8 fail to clone children inside object elements that use
        ...
        } else // dest.outerHTML = src.outerHTML;
        ...
        } else // IE6-8 fails to return the selected option to the default selected
        // state when cloning options
        if (nodeName === 'option') {
            dest.selected = src.defaultSelected;    // IE6-8 fails to set the defaultValue to the correct value when
                                                    // cloning other types of input fields
        } else // IE6-8 fails to set the defaultValue to the correct value when
        // cloning other types of input fields
    ...
    var creators = DOM._creators, defaultCreator = DOM._defaultCreator, R_TBODY = /<tbody/i;    // IE7- adds TBODY when creating thead/tfoot/caption/col/colgroup elements
...
KISSY.add('dom/ie/input-selection', function (S, DOM) {
    var propHooks = DOM._propHooks;    // S.log("fix selectionEnd/Start !");
                                       // note :
    ...
    function getRangeText(elem, range) {
        if (elem.type == 'textarea') {
            var ret = range.text, testRange = range.duplicate();    // consider end \r\n
            // consider end \r\n
...
KISSY.add('dom/ie/selector', function (S, DOM) {
    ...
    if (!doc.querySelectorAll) {
        DOM._getElementsByClassName = function (cls, tag, context) {
            ...
            var els = context.getElementsByTagName(tag || '*'), ret = [], i = 0, j = 0, len = els.length, el;
            for (; i < len; ++i) {
                el = els[i];
    ...
    DOM._getElementsByTagName = function (tag, context) {
        var ret = S.makeArray(context.getElementsByTagName(tag)), t, i, j, node;
        if (tag === '*') {
            t = [];
            i = 0;
            j = 0;
...
KISSY.add('dom/ie/style', function (S, DOM) {
    var cssProps = DOM._cssProps, IE_VERSION = document.documentMode || S.UA.ie, HUNDRED = 100, doc = S.Env.host.document, docElem = doc && doc.documentElement, OPACITY = 'opacity', STYLE = 'style', RE_POS = /^(top|right|bottom|left)$/, FILTER = 'filter', CURRENT_STYLE = 'currentStyle', RUNTIME_STYLE = 'runtimeStyle', LEFT = 'left', PX = 'px', CUSTOM_STYLES = DOM._CUSTOM_STYLES, backgroundPosition = 'backgroundPosition', R_OPACITY = /opacity\s*=\s*([^)]*)/, R_ALPHA = /alpha\([^)]*\)/i;
        ...
        if (docElem.style[OPACITY] == null) {
            CUSTOM_STYLES[OPACITY] = {
                ...
                set: function (elem, val) {
                    val = parseFloat(val);
                    var style = elem[STYLE], currentStyle = elem[CURRENT_STYLE], opacity = isNaN(val) ? '' : 'alpha(' + OPACITY + '=' + val * HUNDRED + ')', filter = S.trim(currentStyle && currentStyle[FILTER] || style[FILTER] || '');    // ie  has layout
                    // ie  has layout
                    style.zoom = 1;    // if setting opacity to 1, and no other filters exist - attempt to remove filter attribute
                                       // https://github.com/kissyteam/kissy/issues/231
    ...
    /*
     ...
     (default is 'medium')
     */
    var IE8 = IE_VERSION == 8, BORDER_MAP = {}, BORDERS = [
            '',
            'Top',
            'Left',
            'Right',
    ...
    S.each(BORDERS, function (b) {
        ...
        CUSTOM_STYLES[name] = {
            get: function (elem, computed) {
                ...
                var currentStyle = computed ? elem[CURRENT_STYLE] : 0, current = currentStyle && String(currentStyle[name]) || undefined;    // look up keywords if a border exists
    ...
    DOM._getComputedStyle = function (elem, name) {
        name = cssProps[name] || name;    // currentStyle maybe null
...
KISSY.add('event/base/object', function (S) {
    ...
    EventObject.prototype = {
        ...
        stopImmediatePropagation: function () {
            var self = this;
            self.isImmediatePropagationStopped = TRUE_FN;    // fixed 1.2
...
KISSY.add('event/base/observer', function (S) {
    ...
    Observer.prototype = {
        ...
        notify: function (event, ce) {
            var ret, self = this, _ks_groups = event._ks_groups;    // handler's group does not match specified groups (at fire step)
...
KISSY.add('event/custom/api-impl', function (S, api, Event, ObservableCustomEvent) {
    var trim = S.trim, _Utils = Event._Utils, splitAndRun = _Utils.splitAndRun, KS_BUBBLE_TARGETS = '__~ks_bubble_targets';
    return S.mix(api, {
        /**
        ...
        fire: function (target, type, eventData) {
            ...
            splitAndRun(type, function (type) {
                var r2, customEvent, typedGroups = _Utils.getTypedGroups(type), _ks_groups = typedGroups[1];
                type = typedGroups[0];
                if (_ks_groups) {
                    _ks_groups = _Utils.getGroupsRe(_ks_groups);
                    eventData._ks_groups = _ks_groups;
                }
                customEvent = ObservableCustomEvent.getCustomEvent(self, type) || // in case no publish custom event but we need bubble
        ...
        removeTarget: function (target, anotherTarget) {
            var targets = api.getTargets(target), index = S.indexOf(anotherTarget, targets);
            if (index != -1) {
        ...
        on: function (target, type, fn, context) {
            ...
            _Utils.batchForType(function (type, fn, context) {
                var cfg = _Utils.normalizeParam(type, fn, context), customEvent;
                type = cfg.type;
        ...
        detach: function (target, type, fn, context) {
            type = trim(type);
            _Utils.batchForType(function (type, fn, context) {
                var cfg = _Utils.normalizeParam(type, fn, context), customEvents, customEvent;
                type = cfg.type;
                if (type) {
                    customEvent = ObservableCustomEvent.getCustomEvent(target, type, 1);
                    if (customEvent) {
...
KISSY.add('event/custom/observable', function (S, api, CustomEventObserver, CustomEventObject, Event) {
    ...
    S.extend(ObservableCustomEvent, Event._ObservableEvent, {
        ...
        checkMemory: function () {
            var self = this, currentTarget = self.currentTarget, events = ObservableCustomEvent.getCustomEvents(currentTarget);
            if (events) {
                if (!self.hasObserver()) {
        ...
        fire: function (eventData) {
            ...
            eventData = eventData || {};
            var self = this, type = self.type, defaultFn = self.defaultFn, i, parents, len, currentTarget = self.currentTarget, customEvent = eventData, gRet, ret;
            eventData.type = type;
            ...
            if (self.bubbles) {
                ...
                for (i = 0; i < len && !customEvent.isPropagationStopped(); i++) {
                    ret = api.fire(parents[i], type, customEvent);    // false 优先返回
                    // false 优先返回
                    if (gRet !== false && ret !== undefined) {
                        gRet = ret;
                    }
            ...
            if (defaultFn && !customEvent.isDefaultPrevented()) {
                ...
                if (!self.defaultTargetOnly && !lowestCustomEvent.defaultTargetOnly || self == customEvent.target) {
                    defaultFn.call(self);
                }
            }
            return gRet;
        ...
        notify: function (event) {
            var observers = this.observers, ret, gRet, len = observers.length, i;
    ...
    ObservableCustomEvent.getCustomEvent = function (target, type, create) {
        var self = this, customEvent, customEvents = ObservableCustomEvent.getCustomEvents(target, create);
...
KISSY.add('event/dom/base/api', function (S, Event, DOM, special, Utils, ObservableDOMEvent, DOMEventObject) {
    ...
    function fixType(cfg, type) {
        var s = special[type] || {};    // in case overwritten by delegateFix/onFix in special events
    ...
    function addInternal(currentTarget, type, cfg) {
        var eventDesc, customEvent, events, handle;
        cfg = S.merge(cfg);
        ...
        if (!(handle = eventDesc.handle)) {
            handle = eventDesc.handle = function (event) {
                ...
                var type = event.type, customEvent, currentTarget = handle.currentTarget;
                if (ObservableDOMEvent.triggeredEvent == type || typeof KISSY == 'undefined') {
                    return;
                }
    ...
    function removeInternal(currentTarget, type, cfg) {
        ...
        var eventDesc = ObservableDOMEvent.getCustomEvents(currentTarget), events = (eventDesc || {}).events;
        if (!eventDesc || !events) {
            return;
    ...
    S.mix(Event, {
        ...
        add: function (targets, type, fn, context) {
            type = S.trim(type);    // data : 附加在回调后面的数据，delegate 检查使用
        ...
        fire: function (targets, eventType, eventData, onlyHandlers)
            /*internal usage*/
            {
                var ret = undefined;    // custom event firing moved to target.js
             ...
             */
                ...
                _Utils.splitAndRun(eventType, function (eventType) {
                    ...
                    var r, i, target, customEvent, typedGroups = _Utils.getTypedGroups(eventType), _ks_groups = typedGroups[1];
                    if (_ks_groups) {
                        _ks_groups = _Utils.getGroupsRe(_ks_groups);
                    }
                    ...
                    for (i = targets.length - 1; i >= 0; i--) {
                        target = targets[i];
                        customEvent = ObservableDOMEvent.getCustomEvent(target, eventType);    // bubbling
...
KISSY.add('event/dom/base/object', function (S, Event, undefined) {
    ...
    function DOMEventObject(domEvent) {
        ...
        self.originalEvent = domEvent;    // in case dom event has been mark as default prevented by lower dom node
        // in case dom event has been mark as default prevented by lower dom node
        self.isDefaultPrevented = domEvent['defaultPrevented'] || domEvent.returnValue === FALSE || domEvent['getPreventDefault'] && domEvent['getPreventDefault']() ? function () {
            return TRUE;
        } : function () {
            return FALSE;
        };
    ...
    /**
         ...
         */
    function fix(self) {
        var originalEvent = self.originalEvent, l = props.length, prop, ct = originalEvent.currentTarget, ownerDoc = ct.nodeType === 9 ? ct : ct.ownerDocument || doc;    // support iframe
    ...
    function fixMouseWheel(e) {
        var deltaX, deltaY, delta, detail = e.detail;    // ie/webkit
        // ie/webkit
        if (e.wheelDelta) {
        ...
        if (deltaX !== undefined || deltaY !== undefined || delta !== undefined) {
            S.mix(e, {
                /**
    ...
    S.extend(DOMEventObject, Event._Object, {
        ...
        preventDefault: function () {
            var e = this.originalEvent;    // if preventDefault exists run it on the original event
        ...
        stopPropagation: function () {
            var e = this.originalEvent;    // if stopPropagation exists run it on the original event
...
KISSY.add('event/dom/base/observable', function (S, DOM, special, Utils, DOMEventObserver, DOMEventObject, Event) {
    ...
    S.extend(ObservableDOMEvent, Event._ObservableEvent, {
        setup: function () {
            var self = this, type = self.type, s = special[type] || {}, currentTarget = self.currentTarget, eventDesc = Utils.data(currentTarget), handle = eventDesc.handle;    // 第一次注册该事件，dom 节点才需要注册 dom 事件
        ...
        notify: function (event) {
             ...
             */
            var target = event['target'], self = this, currentTarget = self.currentTarget, observers = self.observers, currentTarget0, allObservers = [], ret, gRet, observerObj, i, j, delegateCount = self.delegateCount || 0, len, currentTargetObservers, currentTargetObserver, observer;    // collect delegated observers and corresponding element
                                                                                                                                                                                                                                                                                                  // by jq
        ...
        fire: function (event, onlyHandlers)
            ...
            {
                event = event || {};
                var self = this, eventType = self.type, s = special[eventType];    // TODO bug: when fire mouseenter, it also fire mouseover in firefox/chrome
                // TODO bug: when fire mouseenter, it also fire mouseover in firefox/chrome
                if (s && s['onFix']) {
                    eventType = s['onFix'];
                }
                var customEvent, eventData, currentTarget = self.currentTarget, ret = true;
                ...
                // but we can not call event.halt()
                // because handle will check event.isPropagationStopped
                var cur = currentTarget, t, win = DOM.getWindow(cur.ownerDocument || cur), curDocument = win.document, eventPath = [], eventPathIndex = 0, ontype = 'on' + eventType;    // http://www.w3.org/TR/dom/#dispatching-events
                ...
                do {
                    ...
                    customEvent = ObservableDOMEvent.getCustomEvent(cur, eventType);    // default bubble for html node
                ...
                if (!onlyHandlers && !event.isDefaultPrevented()) {
                    ...
                    try {
                        ...
                        if (ontype && currentTarget[eventType] && (eventType !== 'focus' && eventType !== 'blur' || currentTarget.offsetWidth !== 0) && !S.isWindow(currentTarget)) {
                            ...
                            // 记录当前 trigger 触发
                            ObservableDOMEvent.triggeredEvent = eventType;    // 只触发默认事件，而不要执行绑定的用户回调
                                                                              // 同步触发
         ...
         */
        detach: function (cfg) {
            var groupsRe, self = this, s = special[self.type] || {}, hasSelector = 'selector' in cfg, selector = cfg.selector, context = cfg.context, fn = cfg.fn, currentTarget = self.currentTarget, observers = self.observers, groups = cfg.groups;
            if (!observers.length) {
                return;
            }
            if (groups) {
            ...
            if (fn || hasSelector || groupsRe) {
                ...
                for (i = 0, j = 0, t = []; i < len; ++i) {
                    ...
                    if (context != observerContext || // 指定了函数，函数不相等，保留
                        ...
                        hasSelector && (selector && selector != observer.selector || !selector && !observer.selector) || // 指定了删除的某些组，而该 observer 不属于这些组，保留，否则删除
                        groupsRe && !observer.groups.match(groupsRe)) {
                        t[j++] = observer;
                    } else {
        ...
        checkMemory: function () {
            var self = this, type = self.type, events, handle, s = special[type] || {}, currentTarget = self.currentTarget, eventDesc = Utils.data(currentTarget);
            if (eventDesc) {
                events = eventDesc.events;
                if (!self.hasObserver()) {
...
KISSY.add('event/dom/base/observer', function (S, special, Event) {
    ...
    S.extend(DOMEventObserver, Event._Observer, {
        ...
        notifyInternal: function (event, ce) {
            var self = this, s, t, ret, type = event.type;    // restore originalType if involving delegate/onFix handlers
...
KISSY.add('event/dom/base/utils', function (S, DOM) {
    var EVENT_GUID = 'ksEventTargetId_1.30', doc = S.Env.host.document, simpleAdd = doc && doc.addEventListener ? function (el, type, fn, capture) {
...
KISSY.add('event/dom/base/valuechange', function (S, Event, DOM, special) {
    ...
    function monitor(target) {
        unmonitored(target);
        Event.on(target, 'blur', stopPollHandler);    // fix #94
                                                      // see note 2012-02-08
        // fix #94
        // see note 2012-02-08
...
KISSY.add('event/dom/focusin', function (S, Event) {
    var special = Event._Special;    // 让非 IE 浏览器支持 focusin/focusout
...
KISSY.add('event/dom/hashchange', function (S, Event, DOM) {
    var UA = S.UA, special = Event._Special, win = S.Env.host, doc = win.document, docMode = doc && doc['documentMode'], REPLACE_HISTORY = '__replace_history_' + S.now(), ie = docMode || UA['ie'], HASH_CHANGE = 'hashchange';
    Event.REPLACE_HISTORY = REPLACE_HISTORY;    // 1. 不支持 hashchange 事件，支持 hash 历史导航(opera??)：定时器监控
    ...
    var POLL_INTERVAL = 50, IFRAME_TEMPLATE = '<html><head><title>' + (doc && doc.title || '') + ' - {hash}</title>{head}</head><body>{hash}</body></html>', getHash = function () {
        ...
        lastHash, poll = function () {
            ...
            if (replaceHistory = S.endsWith(hash, REPLACE_HISTORY)) {
                hash = hash.slice(0, -REPLACE_HISTORY.length);    // 去除 ie67 hack 标记
                // 去除 ie67 hack 标记
                location.hash = hash;
            }
            if (hash !== lastHash) {
                ...
                lastHash = hash;    // ie<8 同步 : hashChange -> onIframeLoad
        ...
        setup = function () {
            if (!iframe) {
                ...
                Event.add(iframe, 'load', function () {
                    Event.remove(iframe, 'load');    // Update the iframe with the initial location hash, if any. This
                ...
                function onIframeLoad() {
                    ...
                    // #/x?z=1&y=2 => #/x?z=1&amp;y=2
                    var c = S.trim(getIframeDoc(iframe).body.innerText), ch = getHash();    // 后退时不等
                                                                                            // 定时器调用 hashChange() 修改 iframe 同步调用过来的(手动改变 location)则相等
                    // 后退时不等
                    // 定时器调用 hashChange() 修改 iframe 同步调用过来的(手动改变 location)则相等
                    if (c != ch) {
                        S.log('set loc hash :' + c);
                        location.hash = c;    // 使 last hash 为 iframe 历史， 不然重新写iframe，
                                              // 会导致最新状态（丢失前进状态）
                                              // 后退则立即触发 hashchange，
                                              // 并更新定时器记录的上个 hash 值
...
KISSY.add('event/dom/ie/change', function (S, Event, DOM) {
    var special = Event._Special, R_FORM_EL = /^(?:textarea|input|select)$/i;
...
KISSY.add('event/dom/ie/submit', function (S, Event, DOM) {
    var special = Event._Special, getNodeName = DOM.nodeName;
    special['submit'] = {
        setup: function () {
            var el = this;    // form use native
            // form use native
    ...
    function submitBubble(e) {
        var form = this;
        if (form.parentNode && // it is stopped by user callback
...
KISSY.add('event/dom/shake', function (S, EventDomBase, undefined) {
    var Special = EventDomBase._Special, start = 5, enough = 20, shaking = 0, lastX, lastY, lastZ, max = Math.max, abs = Math.abs, win = S.Env.host, devicemotion = 'devicemotion', checkShake = S.buffer(function () {
            if (shaking) {
 ...
 * @author yiminghe@gmail.com
 */
KISSY.add('event/dom/touch/double-tap', function (S, eventHandleMap, Event, SingleTouch) {
    var SINGLE_TAP = 'singleTap', DOUBLE_TAP = 'doubleTap',
...
KISSY.add('event/dom/touch/gesture', function (S, EventDomBase) {
    var Gesture = EventDomBase.Gesture, Features = S.Features, startEvent, moveEvent, endEvent;    // 不能同时绑定 touchstart 与 mousedown 会导致 ios 不能选择文本
...
KISSY.add('event/dom/touch/handle', function (S, DOM, eventHandleMap, Event, Gesture) {
    'use strict';
    var key = S.guid('touch-handle'), Features = S.Features, MOVE_DELAY = 30, touchEvents = {};
    ...
    return {
        addDocumentHandle: function (el, event) {
            var win = DOM.getWindow(el.ownerDocument || el), doc = win.document, setup = eventHandleMap[event].setup, handle = DOM.data(doc, key);
            if (!handle) {
        ...
        removeDocumentHandle: function (el, event) {
            var win = DOM.getWindow(el.ownerDocument || el), doc = win.document, tearDown = eventHandleMap[event].tearDown, handle = DOM.data(doc, key);
            if (tearDown) {
    ...
    S.extend(Pinch, MultiTouch, {
        onTouchMove: function (e) {
            ...
            if (!self.isStarted) {
                ...
                Event.fire(target, PINCH_START, S.mix(e, {
                    distance: distance,
                    scale: 1
                }));
            } else {
                Event.fire(self.target, PINCH, S.mix(e, {
                    distance: distance,
        ...
        fireEnd: function (e) {
            var self = this;
            Event.fire(self.target, PINCH_END, S.mix(e, { touches: self.lastTouches }));
        }
    });
    var p = new Pinch();
    ...
    S.extend(Rotate, MultiTouch, {
        onTouchMove: function (e) {
            ...
            if (lastAngle !== undefined) {
                ...
                else // process '>' scenario: bottom -> top
        ...
        fireEnd: function (e) {
            var self = this;
            Event.fire(self.target, ROTATE_END, S.mix(e, { touches: self.lastTouches }));
        }
    });
    function preventTwoFinger(e) {
        // android can not throttle
...
KISSY.add('event/dom/touch/swipe', function (S, eventHandleMap, Event, SingleTouch) {
    ...
    var MAX_DURATION = 1000, MAX_OFFSET = 35, MIN_DISTANCE = 50;
...
KISSY.add('event/dom/touch/tap', function (S, eventHandleMap, Event, SingleTouch) {
    ...
    S.extend(Tap, SingleTouch, {
        ...
        onTouchEnd: function (e) {
            Event.fire(e.target, event, { touch: e.changedTouches[0] });
        }
    });
...
KISSY.add('json/json2', function () {
    ...
    JSON.stringify = function (value, replacer, space) {
        ...
        } else // If the space parameter is a string, it will be used as the indent string.
...
KISSY.add('ajax/base', function (S, JSON, Event, undefined) {
    var rlocalProtocol = /^(?:about|app|app\-storage|.+\-extension|file|widget)$/, rspace = /\s+/, mirror = function (s) {
            return s;
        }, Promise = S.Promise, rnoContent = /^(?:GET|HEAD)$/, win = S.Env.host, location = win.location || {}, simulatedLocation = /**
         @type KISSY.Uri
         @ignore*/
        new S.Uri(location.href), isLocal = simulatedLocation && rlocalProtocol.test(simulatedLocation.getScheme()), transports = {}, defaultConfig = {
            type: 'GET',
    ...
    function setUpConfig(c) {
        ...
        if (!('cache' in c) && S.inArray(dataType[0], [
                'script',
                'jsonp'
            ])) {
            c.cache = false;
    ...
    function IO(c) {
        ...
        var dataType = c.dataType[0], timeoutTimer, i, timeout = c.timeout, context = c.context, headers = c.headers, accepts = c.accepts;    // Set the Accepts header for the server, depending on the dataType
        ...
        try {
            ...
            if (self.state < 2) {
                S.log(e.stack || e, 'error');
                self._ioReady(-1, e.message);    // Simply rethrow otherwise
            } else
...
KISSY.add('ajax/form', function (S, IO, DOM, FormSerializer) {
    IO.on('start', function (e) {
        var io = e.io, form, d, enctype, dataType, formParam, data, tmpForm, c = io.config;    // serialize form if needed
...
KISSY.add('ajax/iframe-transport', function (S, DOM, Event, IO) {
    'use strict';
    var doc = S.Env.host.document, OK_CODE = 200, ERROR_CODE = 500, BREATH_INTERVAL = 30;    // iframe 内的内容就是 body.innerText
    ...
    function createIframe(xhr) {
        var id = S.guid('io-iframe'), iframe,
    ...
    function addDataToForm(query, form, serializeArray) {
        ...
        S.each(query, function (data, k) {
            ...
            vs = S.makeArray(data);    // 数组和原生一样对待，创建多个同名输入域
            // 数组和原生一样对待，创建多个同名输入域
                ...
                if (eventType == 'load') {
                    try {
                        ...
                        if (iframeDoc && iframeDoc.body) {
                            io.responseText = S.trim(DOM.text(iframeDoc.body));    // ie still can retrieve xml 's responseText
...
KISSY.add('ajax/jsonp', function (S, IO) {
    ...
    IO.on('start', function (e) {
        ...
        if (dataType[0] == 'jsonp') {
            var response, cJsonpCallback = c.jsonpCallback, converters, jsonpCallback = S.isFunction(cJsonpCallback) ? cJsonpCallback() : cJsonpCallback, previous = win[jsonpCallback];
            ...
            io.fin(function () {
                ...
                } else //S.log('delete window variable error : ');
                //S.log(e);
                if (response) {
...
KISSY.add('ajax/methods', function (S, IO, undefined) {
    var OK_CODE = 200, Promise = S.Promise, MULTIPLE_CHOICES = 300, NOT_MODIFIED = 304,
        // get individual response header from response header str
        rheaders = /^(.*?):[ \t]*([^\r\n]*)\r?$/gm;
    function handleResponseData(io) {
        // text xml 是否原生转化支持
        var text = io.responseText, xml = io.responseXML, c = io.config, cConverts = c.converters, xConverts = io.converters || {}, type, contentType, responseData, contents = c.contents, dataType = c.dataType;    // 例如 script 直接是js引擎执行，没有返回值，不需要自己处理初始返回值
        ...
        if (text || xml) {
            ...
            else // 有 xml 值才直接取，否则可能还要从 xml 转
            if (dataType[0] == 'xml' && xml !== undefined) {
                responseData = xml;
            } else {
                var rawData = {
                        text: text,
                ...
                S.each([
                    ...
                    var type = dataType[0], converter = xConverts[prevType] && xConverts[prevType][type] || cConverts[prevType] && cConverts[prevType][type];
        ...
        for (var i = 1; i < dataType.length; i++) {
            type = dataType[i];
            var converter = xConverts[prevType] && xConverts[prevType][type] || cConverts[prevType] && cConverts[prevType][type];
            if (!converter) {
    ...
    S.extend(IO, Promise, {
        ...
        _ioReady: function (status, statusText) {
            ...
            defer[isSuccess ? 'resolve' : 'reject']([
                self.responseData,
                statusText,
                self
            ]);
        ...
        _getUrlForSend: function () {
            ...
            var c = this.config, uri = c.uri, originalQuery = S.Uri.getComponents(c.url).query || '', url = uri.toString(c.serializeArray);
...
KISSY.add('ajax/script-transport', function (S, IO, _, undefined) {
    ...
    function ScriptTransport(io) {
        var config = io.config;    // 优先使用 xhr+eval 来执行脚本, ie 下可以探测到（更多）失败状态
    ...
    S.augment(ScriptTransport, {
        send: function () {
            ...
            script.onerror = script.onload = script.onreadystatechange = function (e) {
                e = e || win.event;    // firefox onerror 没有 type ?!
        ...
        _callback: function (event, abort) {
            var self = this, script = self.script, io = self.io, head = self.head;    // 防止重复调用,成功后 abort
            // 防止重复调用,成功后 abort
            if (!script) {
                return;
            }
            if (abort || !script.readyState || /loaded|complete/.test(script.readyState) || event == 'error') {
                script['onerror'] = script.onload = script.onreadystatechange = null;    // Remove the script
                // Remove the script
                ...
                self.head = undefined;    // Callback if not abort
...
KISSY.add('ajax/xdr-flash-transport', function (S, IO, DOM) {
    ...
    S.augment(XdrFlashTransport, {
        ...
        send: function () {
            ...
            _swf(xdr.src || S.Config.base + 'ajax/io.swf', 1, 1);    // 简便起见，用轮训
            // 简便起见，用轮训
            if (!flash) {
...
KISSY.add('ajax/xhr-transport-base', function (S, IO) {
    var OK_CODE = 200, win = S.Env.host,
        ...
        _XDomainRequest = S.UA.ie > 7 && win['XDomainRequest'], NO_CONTENT_CODE = 204, NOT_FOUND_CODE = 404, NO_CONTENT_CODE2 = 1223, XhrTransportBase = { proto: {} }, lastModifiedCached = {}, eTagCached = {};
    ...
    S.mix(XhrTransportBase.proto, {
        sendInternal: function () {
            var self = this, io = self.io, c = io.config, nativeXhr = self.nativeXhr, type = c.type, async = c.async, username, mimeType = io.mimeType, requestHeaders = io.requestHeaders || {}, url = io._getUrlForSend(), xhrFields, ifModifiedKey = getIfModifiedKey(c), cacheValue, i;
            if (ifModifiedKey) {
            ...
            try {
                ...
                if (abort || nativeXhr.readyState == 4) {
                    ...
                    if (abort) {
                        ...
                        var status = nativeXhr.status;    // _XDomainRequest 不能获取响应头
                        ...
                        io.responseText = nativeXhr.responseText;    // Firefox throws an exception when accessing
                        ...
                        try {
                            ...
                            S.log(e);    // We normalize with Webkit giving an empty statusText
    ...
    function XhrTransport(io) {
        ...
        if (crossDomain) {
            ...
            /*
             ...
             http://hacks.mozilla.org/2009/07/cross-site-xmlhttprequest-with-cors/
             */
            if (!XhrTransportBase.supportCORS && (String(xdrCfg.use) === 'flash' || !_XDomainRequest)) {
...
KISSY.add('base/attribute', function (S, undefined) {
    ...
    function getPathNamePair(self, name) {
        ...
        if (// 声明过，那么 xx.yy 当做普通属性
            !declared && name.indexOf('.') !== -1) {
            path = name.split('.');
            name = path.shift();
        }
        return {
    ...
    function setInternal(self, name, value, opts, attrs) {
        opts = opts || {};
        var ret, path, subVal, prevVal, pathNamePair = getPathNamePair(self, name), fullName = name;
        ...
        value = getValueBySubValue(prevVal, path, value);    // check before event
        // check before event
        if (!opts['silent']) {
        ...
        if (!opts['silent']) {
            ...
            if (!attrs) {
                __fireAttrChange(self, '', '*', [prevVal], [value], [fullName], [name]);
            } else {
                attrs.push({
                    prevVal: prevVal,
                    newVal: value,
    ...
    Attribute.prototype = {
        ...
        addAttr: function (name, attrConfig, override) {
            var self = this, attrs = getAttrs(self), cfg = S.clone(attrConfig);
            if (!attrs[name]) {
                attrs[name] = cfg;
            } else {
        ...
        set: function (name, value, opts) {
            ...
            if (S.isPlainObject(name)) {
                ...
                if (attrNames.length) {
                    __fireAttrChange(self, '', '*', prevVals, newVals, subAttrNames, attrNames);
                }
                return self;
            }
        ...
        setInternal: function (name, value, opts) {
            var self = this, setValue,
                ...
                // 因为属性在声明注册前可以直接设置值
                e, attrConfig = ensureNonEmpty(getAttrs(self), name, true), setter = attrConfig['setter'];    // validator check
            // validator check
         ...
         */
        get: function (name) {
            var self = this, dot = '.', path, declared = self.hasAttr(name), attrVals = getAttrVals(self), attrConfig, getter, ret;
        ...
        reset: function (name, opts) {
            ...
            opts = name;
            var attrs = getAttrs(self), values = {};    // reset all
            // reset all
            for (name in attrs) {
    ...
    function getDefAttrVal(self, name) {
        var attrs = getAttrs(self), attrConfig = ensureNonEmpty(attrs, name), valFn = attrConfig.valueFn, val;
    ...
    function validate(self, name, value, all) {
        ...
        var attrConfig = ensureNonEmpty(getAttrs(self), name, true), e, validator = attrConfig['validator'];
...
KISSY.add('anim/background-position', function (S, DOM, Anim, Fx) {
    function numeric(bp) {
        bp = bp.replace(/left|top/g, '0px').replace(/right|bottom/g, '100%').replace(/([0-9\.]+)(\s|\)|$)/g, '$1px$2');
    ...
    S.extend(BackgroundPositionFx, Fx, {
        ...
        interpolate: function (from, to, pos) {
            ...
            return interpolate(from[0], to[0], pos) + unit[0] + ' ' + interpolate(from[1], to[1], pos) + unit[1];
        },
        cur: function () {
            return DOM.css(this.anim.config.el, 'backgroundPosition');
        },
        update: function () {
            var self = this, prop = self.prop, el = self.anim.config.el, from = self.from, to = self.to, val = self.interpolate(from, to, self.pos);
            DOM.css(el, prop, val);
        }
    });
...
KISSY.add('anim/base', function (S, DOM, Event, Easing, AM, Fx, Q) {
    var UA = S.UA, camelCase = DOM._camelCase, NodeType = DOM.NodeType, specialVals = [
            'hide',
            'show',
            'toggle'
        ],
    ...
    function Anim(el, props, duration, easing, complete) {
        ...
        var self = this, config;    // ignore non-exist element
         ...
         */
        self.config = config;
        self._duration = config.duration * 1000;    // domEl deprecated!
        // domEl deprecated!
        self['domEl'] = el;    // self.props = props;
                               // 实例属性
        // self.props = props;
        // 实例属性
        self._backupProps = {};
        self._fxs = {};    // register complete
        // register complete
        self.on('complete', onComplete);
    }
    function onComplete(e) {
        var self = this, _backupProps, complete, config = self.config;    // only recover after complete anim
    ...
    function runInternal() {
        var self = this, config = self.config, _backupProps = self._backupProps, el = config.el, elStyle, hidden, val, prop, specialEasing = config['specialEasing'] || {}, fxs = self._fxs, props = config.props;    // 进入该函数即代表执行（q[0] 已经是 ...）
        // 进入该函数即代表执行（q[0] 已经是 ...）
        saveRunning(self);
        ...
        // 放在前面，设置 overflow hidden，否则后面 ie6  取 width/height 初值导致错误
        // <div style='width:0'><div style='width:100px'></div></div>
        if (el.nodeType == NodeType.ELEMENT_NODE && (props.width || props.height)) {
            ...
            if (DOM.css(el, 'display') === 'inline' && DOM.css(el, 'float') === 'none') {
                if (UA['ie']) {
                    elStyle.zoom = 1;
                } else {
        ...
        for (prop in props) {
            ...
            var to, from, propCfg = {
                ...
                }, fx = Fx.getFx(propCfg);    // hide/show/toggle : special treat!
            ...
            if (S.inArray(val, specialVals)) {
                ...
                if (val == 'hide') {
                    ...
                    from = 0;
                    to = fx.cur();    // prevent flash of content
            ...
            val += '';
            var unit = '', parts = val.match(NUMBER_REG);
            if (parts) {
    ...
    Anim.prototype = {
         ...
         */
        _frame: function () {
            var self = this, prop, config = self.config, end = 1, c, fx, fxs = self._fxs;
            for (prop in fxs) {
                // 当前属性没有结束
        ...
        stop: function (finish) {
            var self = this, config = self.config, queueName = config.queue, prop, fx, fxs = self._fxs;    // already stopped
            // already stopped
            if (!self.isRunning()) {
                // 从自己的队列中移除
                if (queueName !== false) {
    ...
    function savePaused(anim) {
        var el = anim.config.el, paused = DOM.data(el, pausedKey);
        if (!paused) {
    ...
    Anim.stop = function (el, end, clearQueue, queueName) {
        if (// default queue
            queueName === null || // name of specified queue
            typeof queueName == 'string' || // anims not belong to any queue
            queueName === false) {
    ...
    function pauseResumeQueue(el, queueName, action) {
        ...
        S.each(anims, function (anim) {
            if (queueName === undefined || anim.config.queue == queueName) {
                anim[action]();
            }
        });
    }    /**
    ...
    function stopQueue(el, end, clearQueue, queueName) {
        ...
        var allRunning = DOM.data(el, runningKey), anims = S.merge(allRunning);
        S.each(anims, function (anim) {
...
KISSY.add('anim/color', function (S, DOM, Anim, Fx) {
    var HEX_BASE = 16, floor = Math.floor, KEYWORDS = {
        ...
        }, re_RGB = /^rgb\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+)\)$/i, re_RGBA = /^rgba\(([0-9]+)\s*,\s*([0-9]+)\s*,\s*([0-9]+),\s*([0-9]+)\)$/i, re_hex = /^#?([0-9A-F]{1,2})([0-9A-F]{1,2})([0-9A-F]{1,2})$/i, SHORT_HANDS = Anim.SHORT_HANDS, COLORS = [
            'backgroundColor',
            'borderBottomColor',
            'borderLeftColor',
            'borderRightColor',
    ...
    SHORT_HANDS['borderTop'].push('borderTopColor');    //得到颜色的数值表示，红绿蓝数字数组
    //得到颜色的数值表示，红绿蓝数字数组
...
KISSY.add('anim/easing', function () {
    ...
    var PI = Math.PI, pow = Math.pow, sin = Math.sin, BACK_CONST = 1.70158;    /**
...
KISSY.add('anim/fx', function (S, DOM, undefined) {
    ...
    Fx.prototype = {
         ...
         */
        frame: function (end) {
            var self = this, anim = self.anim, endFlag = 0, elapsedTime;
            if (self.finished) {
                return 1;
            }
            var t = S.now(), _startTime = anim._startTime, duration = anim._duration;
         ...
         */
        update: function () {
            var self = this, anim = self.anim, prop = self.prop, el = anim.config.el, from = self.from, to = self.to, val = self.interpolate(from, to, self.pos);
            if (val === undefined) {
                // 插值出错，直接设置为最终值
                if (!self.finished) {
                    self.finished = 1;
         ...
         */
        cur: function () {
            var self = this, prop = self.prop, el = self.anim.config.el;
            if (isAttr(el, prop)) {
                return DOM.attr(el, prop, undefined, 1);
            }
            var parsed, r = DOM.css(el, prop);    // Empty strings, null, undefined and 'auto' are converted to 0,
    ...
    function isAttr(el, prop) {
        ...
        if ((!el.style || el.style[prop] == null) && DOM.attr(el, prop, undefined, 1) != null) {
            return 1;
        }
        return 0;
    }
...
KISSY.add('anim/manager', function (S) {
    ...
    return {
        ...
        runFrames: function () {
            var self = this, done = 1, r, runnings = self.runnings;
            for (r in runnings) {
                done = 0;
                runnings[r]._frame();
            }
...
KISSY.add('anim/queue', function (S, DOM) {
    ...
    function getQueue(el, name, readOnly) {
        name = name || queueKey;
        var qu, quCollection = DOM.data(el, queueCollectionKey);
    ...
    var q = {
            ...
            queue: function (anim) {
                var el = anim.config.el, name = anim.config.queue, qu = getQueue(el, name);
                qu.push(anim);
                if (qu[0] !== processing) {
                    q.dequeue(anim);
                }
            ...
            remove: function (anim) {
                var el = anim.config.el, name = anim.config.queue, qu = getQueue(el, name, 1), index;
                if (qu) {
            ...
            dequeue: function (anim) {
                var el = anim.config.el, name = anim.config.queue, qu = getQueue(el, name, 1), nextAnim = qu && qu.shift();
...
KISSY.add('node/attach', function (S, DOM, Event, NodeList, undefined) {
    ...
    function accessNormIf(fn, self, index, args) {
        // get
        if (args[index] === undefined    // 并且第一个参数不是对象，否则可能是批量设置写
...
KISSY.add('node/base', function (S, DOM, undefined) {
    var AP = Array.prototype, slice = AP.slice, NodeType = DOM.NodeType, push = AP.push, makeArray = S.makeArray, isNodeList = DOM._isNodeList;    /**
    ...
    function NodeList(html, props, ownerDocument) {
        ...
        if (!html) {
            ...
            // create from html
            domNode = DOM.create(html, props, ownerDocument);    // ('<p>1</p><p>2</p>') 转换为 NodeList
...
/*
 ...
 - append/prepend 参数是节点时，如果当前 NodeList 数量 > 1 需要经过 clone，因为同一节点不可能被添加到多个节点中去（NodeList）
 */
KISSY.use('ua,dom,event,node,json,ajax,anim,base,cookie', { sync: true });