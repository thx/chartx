...
if (!document.createElement('canvas').getContext) {
   ...
   */
     ...
     */
                initElement: function (el) {
                    if (!el.getContext) {
                        el.getContext = getContext;    // Remove fallback content. There is no way to hide text nodes so we
   ...
   */
        ...
        contextPrototype.stroke = function (aFill) {
            ...
            lineStr.push(' ">');
            if (!aFill) {
                var lineWidth = this.lineScale_ * this.lineWidth;    // VML cannot correctly render a line if the width is less than 1px.
