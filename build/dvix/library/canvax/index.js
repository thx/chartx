KISSY.add('canvax/animation/Animation', function (S) {
    ...
    Animation.Tween = function (object) {
        ...
        var _onCompleteCallback = null;    // Set all starting values present on the target object
    ...
    Animation.Easing = {
        ...
        Bounce: {
            ...
            InOut: function (k) {
                if (k < 0.5)
                    return Animation.Easing.Bounce.In(k * 2) * 0.5;
    ...
    var Base = {
         ...
         */
            ...
            creatClass: function (r, s, px) {
                ...
                r.superclass = Base.createObject(sp, s);    // add prototype overrides
            ...
            getType: function (obj) {
                ...
                // 早期的webkit内核浏览器实现了已废弃的ecma262v4标准，可以将正则字面量当作函数使用，因此typeof在判定正则时会返回function
                return typeof obj === 'object' || typeof obj === 'function' ? classTypes[Object.prototype.toString.call(obj)] || 'object' : typeof obj;
            }
        };
    return Base;
...
KISSY.add('canvax/display/Bitmap', function (S, Shape, Base) {
    var Bitmap = function (opt) {
        var self = this;
        self.type = 'bitmap';    //TODO:这里不负责做img 的加载，所以这里的img是必须已经准备好了的img元素
        ...
        //bitmap的图片来源，可以是页面上面的img 也可以是某个canvas
...
KISSY.add('canvax/display/DisplayObject', function (S, EventDispatcher, Matrix, Point, Base, HitTestPoint, propertyFactory) {
    var DisplayObject = function (opt) {
        arguments.callee.superclass.constructor.apply(this, arguments);
        var self = this;    //设置默认属性
        //设置默认属性
        self.id = opt.id || null;    //相对父级元素的矩阵
        //相对父级元素的矩阵
        self._transform = null;    //相对stage的全局矩阵
        ...
        var UID = Base.createId(self.type);    //给每个元素添加eventid，EventManager 事件管理器中要用
        //给每个元素添加eventid，EventManager 事件管理器中要用
        self._eventId = UID;    //如果没有id 则 沿用uid
        //如果没有id 则 沿用uid
        if (self.id == null) {
            self.id = UID;
        }
    ...
    Base.creatClass(DisplayObject, EventDispatcher, {
        ...
        _createContext: function (opt) {
            ...
            //用来存取改显示对象所有和显示有关的属性，坐标，样式等。
            //该对象为Coer.propertyFactory()工厂函数生成
            self.context = null;    //如果用户没有传入context设置，就默认为空的对象
            //如果用户没有传入context设置，就默认为空的对象
            opt.context || (opt.context = {});    //提供给Coer.propertyFactory() 来 给 self.context 设置 propertys
            ...
            _contextATTRS = _.extend(_contextATTRS, _context2D_context);    //然后看继承者是否有提供_context 对象 需要 我 merge到_context2D_context中去的
        ...
        clone: function (myself) {
            ...
            newObj.parent = null;
            newObj.stage = null;    //newObj.context= propertyFactory(this.context.$model);
        ...
        _transformHander: function (context, toGlobal) {
            context.transform.apply(context, this._updateTransform().toArray());    //设置透明度
            //设置透明度
        ...
        _updateTransform: function () {
            ...
            this._transform = _transform;    //更新_transform  对应的全局_transformStage 也要滞空，好在下次使用_transformStage的时候能
        ...
        hitTestPoint: function (mouseX, mouseY) {
            ...
            //检测的结果
            var x = mouseX;
            var y = mouseY;    //这个时候如果有对context的set，告诉引擎不需要watch，因为这个是引擎触发的，不是用户
            ...
            //正式开始第一步的矩形范围判断
            if (x >= this._rect.x && x <= this._rect.x + this._rect.width && y >= this._rect.y && y <= this._rect.y + this._rect.height) {
                //那么就在这个元素的矩形范围内
                //return true;
...
KISSY.add('canvax/display/DisplayObjectContainer', function (S, Base, DisplayObject) {
    DisplayObjectContainer = function (opt) {
        ...
        self.children = [];
        self.mouseChildren = [];
        arguments.callee.superclass.constructor.apply(this, arguments);    //所有的容器默认支持event 检测，因为 可能有里面的shape是eventEnable是true的
    ...
    Base.creatClass(DisplayObjectContainer, DisplayObject, {
        ...
        addChildAt: function (child, index) {
            ...
            child.parent = this;    //上报children心跳
            //上报children心跳
            if (this.heartBeat) {
                this.heartBeat({
                    convertType: 'children',
...
KISSY.add('canvax/display/Movieclip', function (S, DisplayObjectContainer, Base) {
    var Movieclip = function (opt) {
        ...
        //是否循环播放,repeat为数字，则表示循环多少次，为true or !运算结果为true 的话表示永久循环
        self.overPlay = opt.overPlay || false;    //是否覆盖播放，为false只播放currentFrame 当前帧,true则会播放当前帧 和 当前帧之前的所有叠加
        //是否覆盖播放，为false只播放currentFrame 当前帧,true则会播放当前帧 和 当前帧之前的所有叠加
    ...
    Base.creatClass(Movieclip, DisplayObjectContainer, {
        ...
        setFrameRate: function (frameRate) {
            ...
            self._frameRate = frameRate;    //根据最新的帧率，来计算最新的间隔刷新时间
        ...
        afterDelChild: function (child, index) {
            //记录下当前帧
            var preFrame = this.currentFrame;    //如果干掉的是当前帧前面的帧，当前帧的索引就往上走一个
        ...
        play: function () {
            ...
            if (!canvax._heartBeat && canvax._taskList.length == 0) {
                //手动启动引擎
                //canvax.__enterFrame();
                canvax.__startEnter();    //requestAnimationFrame( _.bind(canvax.__enterFrame,canvax) );
            }
            //requestAnimationFrame( _.bind(canvax.__enterFrame,canvax) );
            this._push2TaskList();
            this._preRenderTime = new Date().getTime();    //this._next();
        },
        //this._next();
...
KISSY.add('canvax/display/Stage', function (S, DisplayObjectContainer, Base, StageEvent) {
    var Stage = function (opt) {
        var self = this;
        self.type = 'stage';
        self.context2D = null;    //stage正在渲染中
        //stage正在渲染中
...
;
KISSY.add('canvax/index', function (S, DisplayObjectContainer, Stage, Base, StageEvent, propertyFactory, Sprite, Text, Shape, Movieclip, Bitmap, Point, Shapes, Animation, ImagesLoader) {
    var Canvax = function (opt) {
        var self = this;
        self.type = 'canvax';
        self.el = opt.el || null;    //如果这个时候el里面已经有东西了。嗯，也许曾经这个el被canvax干过一次了。
                                     //那么要先清除这个el的所有内容。
        //如果这个时候el里面已经有东西了。嗯，也许曾经这个el被canvax干过一次了。
        //那么要先清除这个el的所有内容。
        self.el.html('');
        self.mouseTarget = null;
        self.dragTarget = null;    //每帧 由 心跳 上报的 需要重绘的stages 列表
        ...
        self._heartBeat = false;    //心跳，默认为false，即false的时候引擎处于静默状态 true则启动渲染
                                    //设置帧率
        //心跳，默认为false，即false的时候引擎处于静默状态 true则启动渲染
        //设置帧率
        self._speedTime = parseInt(1000 / Base.mainFrameRate);
        self._preRenderTime = 0;    //任务列表, 如果_taskList 不为空，那么主引擎就一直跑
        ...
        self._taskList = [];
        self._Event = null;
        self._hoverStage = null;    //为整个项目提供像素检测的容器
        //为整个项目提供像素检测的容器
        self._pixelCtx = null;
        self._isReady = false;    /**
        *交互相关属性
        ...
        * */
        //接触canvas
        self._touching = false;    //正在拖动，前提是_touching=true
        //正在拖动，前提是_touching=true
        self._draging = false;    //当前的鼠标状态
        //当前的鼠标状态
    ...
    Base.creatClass(Canvax, DisplayObjectContainer, {
        init: function () {
            ...
            self.context.height = self.el.height();    //然后创建一个用于绘制激活shape的 stage到activation
            //然后创建一个用于绘制激活shape的 stage到activation
            self._creatHoverStage();    //初始化事件委托到root元素上面
            //初始化事件委托到root元素上面
            self._initEvent();    //创建一个如果要用像素检测的时候的容器
            //创建一个如果要用像素检测的时候的容器
       ...
       */
        ...
        * */
        __mouseHandler: function (event) {
            ...
            var mouseY = event.pageY - self.rootOffset.top;    //stage拥有mouseX and mouseY
            ...
            if (event.type == 'mousemove' || event.type == 'mousedown') {
                ...
                if (self._touching && event.type == 'mousemove' && self.mouseTarget) {
                    ...
                    if (!self._draging) {
                        //begin drag
                        self.mouseTarget.dragBegin && self.mouseTarget.dragBegin(event);    //先把本尊给隐藏了
                        //先把本尊给隐藏了
                        self.mouseTarget.context.visible = false;    //然后克隆一个副本到activeStage
                ...
                if (this.mouseTarget) {
                    ...
                    e.mouseY = this.mouseY;    //dispatch event
                    //dispatch event
        ...
        __getMouseTarget: function (event) {
            ...
            if (oldObj && oldObj != obj || e.type == 'mouseout') {
                ...
                oldObj.dispatchEvent(e);    //this.setCursor("default");
            }
            ...
            if (obj && oldObj != obj && obj._hoverable) {
                ...
                e.type = 'mouseover';
                e.target = e.currentTarget = obj;
                obj.dispatchEvent(e);    //this.setCursor(obj.context.cursor);
            }
        ...
        _dragEnd: function () {
            var self = this;
            self.dragEnd && self.dragEnd(event);    //拖动停止， 那么要先把本尊给显示出来先
            ...
            self.mouseTarget.context.$owner = self.mouseTarget;    //这个时候的target还是隐藏状态呢
            //这个时候的target还是隐藏状态呢
        ...
        setFrameRate: function (frameRate) {
            ...
            Base.mainFrameRate = frameRate;    //根据最新的帧率，来计算最新的间隔刷新时间
            //根据最新的帧率，来计算最新的间隔刷新时间
        ...
        __enterFrame: function () {
            var self = this;    //不管怎么样，__enterFrame执行了就要把
                                //requestAid null 掉
            ...
            if (self._heartBeat) {
                if (Base.now - self._preRenderTime < self._speedTime) {
                    //事件speed不够，下一帧再来
                    self.__startEnter();    //self.requestAid = requestAnimationFrame( _.bind(self.__enterFrame,self) );
                    //self.requestAid = requestAnimationFrame( _.bind(self.__enterFrame,self) );
                    return;
                }
                ...
                self._heartBeat = false;    //debugger;
                //debugger;
                self.convertStages = {};    //渲染完了，打上最新时间挫
                //渲染完了，打上最新时间挫
        ...
        heartBeat: function (opt) {
            //displayList中某个属性改变了
            var self = this;    //console.log("heartBeat")
    ...
    Canvax.propertyFactory = propertyFactory;    //给Canvax 添加静态对象，指向stage ,shape,text,sprite等类
...
;
    ...
    Canvax.Utils = { ImagesLoader: ImagesLoader };
    ...
    requires: [
        ...
        !window._ ? 'canvax/library/underscore' : ''    //如果用户没有加载flashcavnas在ie下面，并且也没有加载excanvas，就默认加载自己准备的flashcanvas进来
...
KISSY.add('canvax/shape/Beziercurve', function (S, Shape, Base) {
    var Beziercurve = function (opt) {
        ...
        //线条只能描边，填充的画出了问题别怪我没提醒
        //在diaplayobject组织context的之前就要把 这个 shape 需要的 style 字段 提供出来
        //displayobject 会merge 到 context去
        opt.context || (opt.context = {});
        self._context = {
            xStart: opt.context.xStart || 0,
            //        : {number} 必须，起点横坐标
            yStart: opt.context.yStart || 0,
            //       : {number},   必须，起点纵坐标
            cpX1: opt.context.cpX1 || 0,
            //       : {number},  // 必须，第一个关联点横坐标
            cpY1: opt.context.cpY1 || 0,
            //      : {number},  // 必须，第一个关联点纵坐标
            cpX2: opt.context.cpX2 || 0,
            //第二个关联点横坐标  缺省即为二次贝塞尔曲线
            cpY2: opt.context.cpY2 || 0,
            //       : {number},  // 可选，第二个关联点纵坐标
            xEnd: opt.context.xEnd || 0,
            //       : {number},  // 必须，终点横坐标
...
KISSY.add('canvax/shape/Droplet', function (S, Shape, Base) {
    var Droplet = function (opt) {
        ...
        //{number},  // 必须，水滴纵高（中心到尖端距离）
        arguments.callee.superclass.constructor.apply(this, arguments);
    };
    Base.creatClass(Droplet, Shape, {
        draw: function (ctx, style) {
            ctx.moveTo(0, style.hr);
            ctx.bezierCurveTo(style.hr, style.hr, style.hr * 3 / 2, -style.hr / 3, 0, -style.vr);
            ctx.bezierCurveTo(-style.hr * 3 / 2, -style.hr / 3, -style.hr, style.hr, 0, style.hr);
            return;
        },
        getRect: function (style) {
            var lineWidth;
...
KISSY.add('canvax/shape/Heart', function (S, Shape, Base) {
    var Heart = function (opt) {
        ...
        self._context = {
            ...
            hr: opt.context.hr || 0,
            //{number},  // 必须，心形横宽（中轴线到水平边缘最宽处距离）
            vr: opt.context.vr || 0    //{number},  // 必须，心形纵高（内尖到外尖距离）
        };
        //{number},  // 必须，心形纵高（内尖到外尖距离）
        arguments.callee.superclass.constructor.apply(this, arguments);
    };
    Base.creatClass(Heart, Shape, {
        draw: function (ctx, style) {
            ctx.moveTo(0, 0);
            ctx.bezierCurveTo(style.hr / 2, -style.vr * 2 / 3, style.hr * 2, style.vr / 3, 0, style.vr);
            ctx.bezierCurveTo(-style.hr * 2, style.vr / 3, -style.hr / 2, -style.vr * 2 / 3, 0, 0);
            return;
        },
        getRect: function (style) {
            var lineWidth;
       ...
       */
        draw: function (ctx, style) {
            if (!style.lineType || style.lineType == 'solid') {
                ...
                this.dashedLineTo(ctx, style.xStart, style.yStart, style.xEnd, style.yEnd, dashLength);
            }
        },
        /**
       * 返回矩形区域，用于局部刷新和文字定位
...
KISSY.add('canvax/shape/Path', function (S, Shape, Base) {
    var Path = function (opt) {
        ...
        //字符串 必须，路径。例如:M 0 0 L 0 10 L 10 10 Z (一个三角形)
        //M = moveto
        //L = lineto
        //H = horizontal lineto
        //V = vertical lineto
    ...
    Base.creatClass(Path, Shape, {
        ...
        draw: function (ctx, style) {
            var path = style.path;
            var pathArray = this._parsePathData(path);    // 平移坐标
            // 平移坐标
            var x = 0;
            var y = 0;
            var p;    // 记录边界点，用于判断inside
            // 记录边界点，用于判断inside
            var $pointList = style.$pointList = [];
            var singlePointList = [];
...
KISSY.add('canvax/shape/Polygon', function (S, Shape, Base) {
    ...
    Base.creatClass(Polygon, Shape, {
        ...
        buildPath: function (ctx, style) {
            var pointList = style.pointList.$model;    // 开始点和结束点重复
            // 开始点和结束点重复
            ...
            if (!style.lineType || style.lineType == 'solid') {
                ...
                ctx.moveTo(pointList[0][0], pointList[0][1]);
                for (var i = 1, l = pointList.length; i < l; i++) {
                    this.dashedLineTo(ctx, pointList[i - 1][0], pointList[i - 1][1], pointList[i][0], pointList[i][1], dashLength);
                }
                this.dashedLineTo(ctx, pointList[pointList.length - 1][0], pointList[pointList.length - 1][1], pointList[0][0], pointList[0][1], dashLength);
            }
            return;
        },
        getRect: function (style) {
...
KISSY.add('canvax/shape/Sector', function (S, Shape, math, Polygon, Base) {
    var Sector = function (opt) {
        ...
        self._context = {
            ...
            r: opt.context.r || 0,
            //{number},  // 必须，外圆半径
            startAngle: opt.context.startAngle || 0,
 ...
 ? // 形内半径[0,r)
            ...
            endAngle = math.degreeToRadian(endAngle);    //sin&cos已经在tool.math.缓存了，放心大胆的重复调用
                                                         //ctx.moveTo(
            ...
            //    y - math.sin(startAngle) * r
            //);
            ctx.arc(x, y, r, PI2 - startAngle, PI2 - endAngle, true);    //ctx.lineTo(
...
KISSY.add('canvax/utils/HitTestPoint', function (S, Base) {
    ...
    function isInside(shape, x, y) {
        ...
        var zoneType = shape.type;    // 未实现或不可用时则数学运算，主要是line，brokenLine
    ...
    function _mathMethod(zoneType, shape, x, y) {
        ...
        //线-----------------------1
        case 'line':
            return _isInsideLine(shape.context, x, y);    //折线----------------------2
        //折线----------------------2
        case 'brokenLine':
            return _isInsideBrokenLine(shape, x, y);    //文本----------------------3
        //文本----------------------3
        case 'text':
            return true;    //圆环----------------------4
        //圆环----------------------4
        case 'ring':
            return _isInsideRing(shape, x, y);    //矩形----------------------5
        //矩形----------------------5
        case 'rect':
            return true;    //圆形----------------------6
        //圆形----------------------6
        case 'circle':
            return _isInsideCircle(shape, x, y);    //椭圆
        //椭圆
        case 'ellipse':
            return _isPointInElipse(shape, x, y);    //扇形----------------------7
        //扇形----------------------7
        case 'sector':
            return _isInsideSector(shape, x, y);    //path---------------------8
        //path---------------------8
        case 'path':
            return _isInsidePath(shape, x, y);    //多边形-------------------9
        //多边形-------------------9
        case 'polygon':
        case 'star':
        case 'isogon':
            return _isInsidePolygon(shape, x, y);    //图片----------------------10
        //图片----------------------10
        case 'image':
            return true;
        }
    ...
    function _buildPathMethod(shape, context, x, y) {
        var area = shape.context;    // 图形类实现路径创建了则用类的path
        // 图形类实现路径创建了则用类的path
        context.beginPath();
    ...
    function _pixelMethod(shape, x, y) {
        ...
        _context.transform.apply(_context, shape.getConcatenatedMatrix().toArray());    //这个时候肯定是做过矩形范围检测过来的
        ...
        _context.closePath();
        _context.restore();    //对鼠标的坐标也做相同的变换
        //对鼠标的坐标也做相同的变换
    ...
    function _isInsideRing(shape, x, y) {
        var area = shape.context;
        if (_isInsideCircle(shape, x, y) && !_isInsideCircle(shape, x, y, area.r0 || 0)) {
            // 大圆内，小圆外
            return true;
        }
        return false;
    ...
    function _isInsideSector(shape, x, y) {
        var area = shape.context;
        if (!_isInsideCircle(shape, x, y) || area.r0 > 0 && _isInsideCircle(shape, x, y, area.r0)) {
            // 大圆外或者小圆内直接false
            return false;
        } else {
            // 判断夹角
            var angle = (360 - Math.atan2(y, x) / Math.PI * 180) % 360;
        ...
        if (redo) {
            ...
            for (i = 0, j = N - 1; i < N; j = i++) {
                if (polygon[i][1] < y && y < polygon[j][1] || polygon[j][1] < y && y < polygon[i][1]) {
                    ...
                    if (x < polygon[i][0]) {
                        // 交点在顶点上
                        polygon[i][1] > polygon[j][1] ? --y : ++y;    //redo = true;
                        //redo = true;
                        break;
                    }
