KISSY.add("dvix/components/tips/Tips" , function(S, Dvix, Tip, Line, Nodes, Tools){
    ...
    Tips.prototype = {
        ...
        _startDraw:function(){
            ...
            if(self.opt.tip){
                self.tip.draw({
                    data  : self.config.tip.data
                })
                var x = self.config.tip.x, y = self.config.tip.y
                var p = self._allShow(self.w, self.h, {w:self.tip.w,h:self.tip.h}, {x:x,y:y}, self.opt.dis)
                x = p.x, y = p.y
