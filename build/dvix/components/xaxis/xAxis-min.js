KISSY.add('dvix/components/xaxis/xAxis', function (a, b, c, d) {
    ...
    return e.prototype = {
        init: function (a, c) {
            this.dataOrg = c.org, a && _.deepExtend(this, a), this.dataSection = this._initDataSection(this.dataOrg), this.sprite = new b.Display.Sprite(), this._checkText();
        },
        ...
        _widget: function () {
            ...
            this.txtSp = new b.Display.Sprite(), this.sprite.addChild(this.txtSp), this.lineSp = new b.Display.Sprite(), this.sprite.addChild(this.lineSp);
            for (var e = 0, f = a.length; f > e; e++) {
                var g = a[e], h = g.x, i = this.disY + this.line.height + this.dis, j = d.numAddSymbol(g.content), k = new b.Display.Text(j, {
                        context: {
                            x: h,
                            y: i,
            ...
            for (var a = 1 == this.text.mode ? this.layoutData : this.data, e = 0, f = a.length; f > e; e++) {
                var g = a[e], h = g.x, l = new c({
                        id: e,
                        context: {
                            x: h,
                            y: 0,
            ...
            for (var e = 0, f = this.txtSp.getNumChildren(); f > e; e++) {
                var k = this.txtSp.getChildAt(e), h = parseInt(k.context.x - k.getTextWidth() / 2);
                k.context.x = h;
            }
        },
        _layout: function () {
        ...
        _trimLayoutData: function () {
            for (var a = [], c = this.data, e = 0, f = 0, g = c.length; g > f; f++) {
                var h = c[f], i = d.numAddSymbol(h.content), j = new b.Display.Text(i, {
                        context: {
                            fillStyle: this.text.fillStyle,
                            fontSize: this.text.fontSize
                        }
                    });
                e = Math.max(e, j.getTextWidth());
            }
            for (var k = this.max.right, l = Math.min(Math.floor(k / e), c.length), m = Math.max(Math.ceil(c.length / l - 1), 0), f = 0; l > f; f++) {
                var n = c[f + m * f];
                n && a.push(n);
            }
            this.layoutData = a;
