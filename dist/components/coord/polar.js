...
(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "./index", "canvax", "./polar/grid", "mmvis"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("./index"), require("canvax"), require("./polar/grid"), require("mmvis"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.index, global.canvax, global.grid, global.mmvis);
    global.undefined = mod.exports;
  }
})(void 0, function (exports, _index, _canvax, _grid, _mmvis) {
  "use strict";
  ...
  var Polar = function (_coorBase) {
    _inherits(Polar, _coorBase);

    _createClass(Polar, null, [{
      key: "defaultProps",
    ...
    function Polar(opt, app) {
      ...
      _this = _possibleConstructorReturn(this, _getPrototypeOf(Polar).call(this, opt, app));
      _this.type = "polar";

      _mmvis._.extend(true, _assertThisInitialized(_this), (0, _mmvis.getDefaultProps)(Polar.defaultProps()), _this.setDefaultOpt(opt, app));
    ...
    _createClass(Polar, [{
      ...
      value: function setDefaultOpt(coordOpt, app) {
        ...
        _mmvis._.extend(true, coord, coordOpt); //根据graphs.field 来 配置 coord.rAxis.field -------------------


        if (!_mmvis._.isArray(coord.rAxis.field)) {
          coord.rAxis.field = [coord.rAxis.field];
        }

        ; //根据opt中得Graphs配置，来设置 coord.yAxis

        var graphsArr = _mmvis._.flatten([app._opt.graphs]); //有graphs的就要用找到这个graphs.field来设置coord.rAxis


        var arrs = [];

        _mmvis._.each(graphsArr, function (graphs) {
          if (graphs.field) {
            //没有配置field的话就不绘制这个 graphs了
            var _fs = graphs.field;

            if (!_mmvis._.isArray(_fs)) {
              _fs = [_fs];
            }
      ...
      value: function _initModules() {
        if (this.grid.enabled) {
          this._grid = new _grid2["default"](this.grid, this);
        ...
        if (this.aAxis.enabled && this.grid.enabled) {
          this._aAxisScaleSp = new _canvax2["default"].Display.Sprite({
            id: "aAxisScaleSp"
          });
      ...
      value: function _getRDataSection() {
        ...
        _mmvis._.each(_mmvis._.flatten([me.rAxis.field]), function (field) {
          arr = arr.concat(me.app.dataFrame.getFieldData(field));
        });

        arr.push(0);
        return _mmvis.dataSection.section(arr, 3);
      }
    }, {
      key: "_computeAttr",
      value: function _computeAttr() {
        ...
        for (var i = 0, l = parseInt((this.startAngle + this.allAngle) / 90) - parseInt(this.startAngle / 90); i <= l; i++) {
          ...
          if (_mmvis._.indexOf(angles, angle) == -1 && angle > angles.slice(-1)[0]) {
            angles.push(angle);
          }
        ...
        if (_mmvis._.indexOf(angles, lastAngle) == -1) {
          angles.push(lastAngle);
        }

        ;

        _mmvis._.each(angles, function (angle) {
          angle = angle % 360;
        ...
        _mmvis._.each(angles, function (angle) {
          ...
          _mmvis._.each(lines, function (line) {
            var _r;
      ...
      value: function getRadiansAtR(r, width, height) {
        ...
        if (_rs.length == 0) {
          ...
          //分割多段
          _mmvis._.each(_rs, function (point, i) {
      ...
      value: function getROfNum(num) {
        ...
        var maxNum = _mmvis._.max(this.rAxis.dataSection);
      ...
      value: function getPointsOfR(r) {
        ...
        _mmvis._.each(me.aAxis.angleList, function (_a) {
          //弧度
      ...
      value: function _setAAxisAngleList() {
        ...
        _mmvis._.each(aAxisArr, function (p) {
          //角度
      ...
      value: function _drawAAxis() {
        //绘制aAxis刻度尺
        var me = this;
        var r = me.getROfNum(_mmvis._.max(this.rAxis.dataSection));
        var points = me.getPointsOfR(r + 3);
        me._aAxisScaleSp.context.x = this.origin.x;
        me._aAxisScaleSp.context.y = this.origin.y;

        _mmvis._.each(this.aAxis.data, function (value, i) {
          var point = points[i];
          var text = _mmvis._.isFunction(me.aAxis.label.format) ? me.aAxis.label.format(value) : value;
          var nodeData = {
            value: value,
            text: text,
            iNode: i,
          ...
          _mmvis._.extend(c, me._getTextAlignForPoint(Math.atan2(point.y, point.x)));

          me._aAxisScaleSp.addChild(new _canvax2["default"].Display.Text(text, {
            context: c
          }));
      ...
      value: function getAAxisIndOf(e) {
        ...
        _mmvis._.each(me.aAxis.angleList, function (_a, i) {
      ...
      value: function _initInduce() {
        ...
        me.induce && me.induce.on(_mmvis.event.types.get(), function (e) {
      ...
      value: function getTipsInfoHandler(e) {
        ...
        if (e.eventInfo) {
          obj = _mmvis._.extend(obj, e.eventInfo);
        }
  ...
  _mmvis.global.registerComponent(Polar, 'coord', 'polar');
