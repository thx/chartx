...
(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["exports", "canvax", "../../../utils/tools", "../index", "mmvis"], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require("canvax"), require("../../../utils/tools"), require("../index"), require("mmvis"));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.canvax, global.tools, global.index, global.mmvis);
    global.undefined = mod.exports;
  }
})(void 0, function (exports, _canvax, _tools, _index2, _mmvis) {
  "use strict";
  ...
  var BarGraphs = function (_GraphsBase) {
    ...
    function BarGraphs(opt, app) {
      ...
      _mmvis._.extend(true, _assertThisInitialized(_this), (0, _mmvis.getDefaultProps)(BarGraphs.defaultProps()), opt);
    ...
    _createClass(BarGraphs, [{
      key: "init",
      value: function init() {
        this.barsSp = new _canvax2["default"].Display.Sprite({
          id: "barsSp"
        });
        this.txtsSp = new _canvax2["default"].Display.Sprite({
          id: "txtsSp",
      ...
      value: function _getTargetField(b, v, i, field) {
        if (_mmvis._.isString(field)) {
          return field;
        } else if (_mmvis._.isArray(field)) {
          var res = field[b];

          if (_mmvis._.isString(res)) {
            return res;
          } else if (_mmvis._.isArray(res)) {
            return res[v];
          }
      ...
      value: function _getColor(color, nodeData) {
        ...
        if (_mmvis._.isArray(color)) {
          color = _mmvis._.flatten(color)[_mmvis._.indexOf(_flattenField, field)];
        }
        ...
        if (color && color.lineargradient && color.lineargradient.length) {
          if (nodeData.rectHeight != 0) {
            ...
            _mmvis._.each(color.lineargradient, function (item) {
      ...
      value: function _getBarWidth(cellWidth, ceilWidth2) {
        if (this.node.width) {
          if (_mmvis._.isFunction(this.node.width)) {
      ...
      value: function hide(field) {
        _mmvis._.each(this.barsSp.children, function (h_groupSp, h) {
          var _bar = h_groupSp.getChildById("bar_" + h + "_" + field);

          _bar && _bar.destroy();
        });

        _mmvis._.each(this.txtsSp.children, function (sp, h) {
      ...
      value: function draw(opt) {
        ...
        _mmvis._.each(barGraphs, function (graph, i) {
          if (graph == me) {
            preGraphs = i;
          }
        ...
        _mmvis._.each(this.enabledField, function (h_group, i) {
          h_group = _mmvis._.flatten([h_group]);
          /*
          ...
          */
          ...
          function barGroupSectedHandle(e) {
            if (me.select.enabled && e.type == me.select.triggerEventType) {
              ...
              if (_mmvis._.indexOf(me.select.inds, ind) > -1) {
                //说明已经选中了
                _mmvis._.each(barGraphs, function (barGraph) {
          ...
          for (var h = 0; h < me._dataLen; h++) {
            ...
            if (i == 0) {
              ...
              if (h <= preDataLen - 1) {
                ...
                groupH = new _canvax2["default"].Display.Sprite({
                  id: "barGroup_" + h
                });
              ...
              if (!preGraphs) {
                ...
                if (h <= preDataLen - 1) {
                  ...
                  groupRegion.context.globalAlpha = _mmvis._.indexOf(me.select.inds, me.dataFrame.range.start + h) > -1 ? me.select.alpha : 0;
                } else {
                  groupRegion = new Rect({
                    ...
                    context: {
                      ...
                      globalAlpha: _mmvis._.indexOf(me.select.inds, me.dataFrame.range.start + h) > -1 ? me.select.alpha : 0
                    }
                  });
                  groupH.addChild(groupRegion);
                  groupRegion.iNode = h; //触发注册的事件

                  groupRegion.on(_mmvis.event.types.get(), function (e) {
                    e.eventInfo = {
            ...
            if (i == 0) {
              if (h <= preDataLen - 1) {
                ...
                txtGroupH = new _canvax2["default"].Display.Sprite({
                  id: "txtGroup_" + h
                });
            ...
            for (var v = 0; v < vLen; v++) {
              ...
              if (rectEl) {
                ...
                rectEl.on(_mmvis.event.types.get(), function (e) {
                  e.eventInfo = {
                    trigger: me.node,
                    nodes: [this.nodeData]
                  };
              ...
              if (me.label.enabled) {
                ...
                if (textEl) {
                  ...
                  textEl = new _canvax2["default"].Display.Text(value, {
                    id: textId,
                    context: textCtx
                  });
      ...
      value: function _getGroupRegionStyle(iNode) {
        ...
        if (_mmvis._.isArray(me.select.fillStyle)) {
          _groupRegionStyle = me.select.fillStyle[iNode];
        }

        ;

        if (_mmvis._.isFunction(me.select.fillStyle)) {
          ...
          _mmvis._.each(hData, function (vSectionData, v) {
            ...
            _mmvis._.each(vSectionData, function (val, i) {
              ...
              if (me.proportion) {
                ...
                _mmvis._.each(hData, function (team, ti) {
                  vCount += team[i];
                });
              }
              ...
              if (_mmvis._.indexOf(me.select.inds, i) > -1) {
      ...
      value: function grow(callback, opt) {
        ...
        _mmvis._.each(me.enabledField, function (h_group, g) {
          h_group = _mmvis._.flatten([h_group]);
          var vLen = h_group.length;
          if (vLen == 0) return;
      ...
      value: function selectAt(ind) {
        var me = this;
        if (_mmvis._.indexOf(this.select.inds, ind) > -1) return;
        this.select.inds.push(ind); //因为这里是带上了start的全局的index，

        var index = ind - this.dataFrame.range.start;

        _mmvis._.each(this.data, function (list, f) {
      ...
      value: function unselectAt(ind) {
        ...
        var _index = _mmvis._.indexOf(this.select.inds, ind);
      ...
      value: function getSelectedRowList() {
        ...
        _mmvis._.each(me.select.inds, function (ind) {
  ...
  _mmvis.global.registerComponent(BarGraphs, 'graphs', 'bar');

  exports["default"] = BarGraphs;
});