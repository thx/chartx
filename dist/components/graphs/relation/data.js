...
(function (global, factory) {
  ...
  exports.arrayToTreeJsonForRelation = exports.jsonToArrayForRelation = exports.checkDataIsJson = undefined;
  ...
  function checkDataIsJson(data, key, _childrenField) {
    ...
    if (!_mmvis._.isArray(data)) return false; //2、数组中的每个元素是一个对象

    result = _mmvis._.every(data, function (item) {
      return _mmvis._.isObject(item);
    }); //3、field 自动不能是数组，如果是表示已经处理过的数据

    if (result) {
      result = _mmvis._.every(data, function (item) {
        return !_mmvis._.isArray(item[key]);
      });
    }

    ; //4、至少有一个元素中存在关键字

    result = _mmvis._.some(data, function (item) {
  ...
  function jsonToArrayForRelation(data, options, _childrenField) {
    ...
    _mmvis._.each(data, function (item) {
      childrens.push(item);
    });
    ...
    var _loop = function _loop() {
      ...
      if (_child) {
        _mmvis._.each(_child, function (ch) {
          wm.set(ch, {
            parentIndex: index,
            parentNode: item
          });
      ...
      _mmvis._.each(item, function (value, key) {
        if (key !== childrenKey) {
          obj[key] = value;
        }
      });
  ...
  function arrayToTreeJsonForRelation(data, options) {
    ...
    _mmvis._.each(_nodes, function (node, nkey) {
      var isFirstLev = true;

      _mmvis._.each(_edges, function (edge, ekey) {
