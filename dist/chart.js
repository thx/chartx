...
(function (global, factory) {
  ...
  var Chart = function (_event$Dispatcher) {
    ...
    function Chart(node, data, opt, componentModules) {
      ...
      _this.el = _mmvis.$.query(node); //chart 在页面里面的容器节点，也就是要把这个chart放在哪个节点里
      ...
      _this.canvax = new _canvax2["default"].App({
        el: _this.el,
        webGL: false
      });
      ...
      _this.stage = new _canvax2["default"].Display.Stage({
        id: "main-chart-stage"
      });
    ...
    _createClass(Chart, [{
      ...
      value: function init() {
        ...
        _mmvis._.each(this.__highModules, function (compName) {
          if (!opt[compName]) return;

          var comps = _mmvis._.flatten([opt[compName]]); //them是一个数组的组件。so特殊处理
          ...
          _mmvis._.each(comps, function (comp) {
        ...
        for (var _p in this._opt) {
          ...
          if (_mmvis._.indexOf(this.__highModules, _p) == -1) {
            var comps = this._opt[_p]; //所有的组件都按照数组方式处理，这里，组件里面就不需要再这样处理了

            if (!_mmvis._.isArray(comps)) {
              comps = [comps];
            }
      ...
      value: function draw(opt) {
        ...
        var completeNum = 0;
        opt = _mmvis._.extend(opt, {
          width: width,
          height: height,
          origin: origin
        });

        _mmvis._.each(_graphs, function (_g) {
        ...
        for (var i = 0, l = this.components.length; i < l; i++) {
          ...
          if (_mmvis._.indexOf(this.__highModules, p.name) == -1) {
            p.draw(opt);
          }
      ...
      value: function _horizontalGraphsText() {
        ...
        function _horizontalText(el) {
          if (el.children) {
            _mmvis._.each(el.children, function (_el) {
              _horizontalText(_el);
            });
          }
        ...
        _mmvis._.each(me.getComponents({
          name: 'graphs'
        }), function (_graphs) {
      ...
      value: function _getPadding() {
        ...
        if (this._opt.coord && "padding" in this._opt.coord) {
          if (!_mmvis._.isObject(this._opt.coord.padding)) {
        ...
        if (this._opt.coord && "padding" in this._opt.coord) {
          if (_mmvis._.isObject(this._opt.coord.padding)) {
            paddingObj = _mmvis._.extend(paddingObj, this._opt.coord.padding);
          }
        }
      ...
      value: function getTheme(ind) {
        var colors = _mmvis.global.getGlobalTheme();
      ...
      value: function setCoord_Graphs_Sp() {
        //坐标系存放的容器
        this.coordSprite = new _canvax2["default"].Display.Sprite({
          id: 'coordSprite'
        });
        this.stage.addChild(this.coordSprite); //graphs管理

        this.graphsSprite = new _canvax2["default"].Display.Sprite({
          id: 'graphsSprite'
        });
      ...
      value: function resetData(data, trigger) {
        ...
        _mmvis._.each(this.getComponents({
          name: 'graphs'
        }), function (_g) {
      ...
      value: function initData() {
        return _mmvis.dataFrame.apply(this, arguments);
      }
    }, {
      key: "componentsReset",
      value: function componentsReset(trigger) {
        ...
        _mmvis._.each(this.components, function (p, i) {
          //theme coord graphs额外处理
          if (_mmvis._.indexOf(me.__highModules, p.name) != -1) {
            return;
          }
      ...
      value: function getComponentById(id) {
        ...
        _mmvis._.each(this.components, function (c) {
          if (c.id && c.id == id) {
            comp = c;
            return false;
          }
      ...
      value: function getComponents(opt, components) {
        ...
        _mmvis._.each(components, function (comp) {
          var i = 0;
      ...
      value: function getCoord(opt) {
        return this.getComponent(_mmvis._.extend(true, {
          name: 'coord'
        }, opt));
      }
    }, {
      ...
      value: function getLegendData() {
        ...
        _mmvis._.each(this.getComponents({
          ...
          _mmvis._.each(_g.getLegendData(), function (item) {
            if (_mmvis._.find(data, function (d) {
              return d.name == item.name;
            })) return;

            var legendItem = _mmvis._.extend(true, {}, item);
        ...
        _mmvis._.each(_mmvis._.flatten(_coord.fieldsMap), function (map, i) {
          //因为yAxis上面是可以单独自己配置field的，所以，这部分要过滤出 legend data
          var isGraphsField = false;

          _mmvis._.each(me._opt.graphs, function (gopt) {
            if (_mmvis._.indexOf(_mmvis._.flatten([gopt.field]), map.field) > -1) {
              isGraphsField = true;
              return false;
            }
          });
      ...
      value: function _bindEvent() {
        var me = this;
        this.on(_mmvis.event.types.get(), function (e) {
          //触发每个graphs级别的事件，
          //用户交互事件先执行，还可以修改e的内容修改tips内容
          if (e.eventInfo) {
            _mmvis._.each(this.getGraphs(), function (graph) {
  ...
  _mmvis.global.registerComponent(Chart, 'chart');
