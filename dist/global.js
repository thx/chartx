...
var _default = {
  create: function create(el, _data, _opt) {
    ...
    var chart_id = _canvax.$.query(el).getAttribute("chart_id");
  ...
  */
  getOptions: function getOptions(chartPark_cid, userOptions, data, variables) {
    ...
    if (userOptions) {
      optionsFromChartPark = _canvax._.extend(true, optionsFromChartPark, userOptions);
    }
  ...
  getProps: function getProps() {
    ...
    var _loop = function _loop(n) {
      ...
      if (n == 'graphs') {
        _graphNames = _canvax._.map(allModules.graphs, function (val, key) {
          return key;
        });
      ...
      function _setProps(m) {
        ...
        if (constructorModule.defaultProps && _canvax._.isFunction(constructorModule.defaultProps)) {
          var _dprops = constructorModule.defaultProps();

          _canvax._.extend(allConstructorProps, _dprops);
        }
      ...
      allProps[n].propertys = _canvax._.extend(allConstructorProps, allProps[n].propertys);
      ...
      var _loop2 = function _loop2(_mn) {
        ...
        function setChildProps(p) {
          if (p._props) {
            var _propsIsArray = _canvax._.isArray(p._props);
              ...
              if (_module.defaultProps) {
                (function () {
                  var _setProps = function _setProps(m) {
                    ...
                    if (m.defaultProps && _canvax._.isFunction(m.defaultProps)) {
                      ...
                      ;
                      _dprops && _canvax._.extend(allConstructorProps, _dprops);
                    }
                  ...
                  _moduleProps = _canvax._.extend(allConstructorProps, _moduleProps);

                  if (_propsIsArray) {
                    _canvax._.extend(p, _moduleProps);
                  } else {
        ...
        moduleProps = _canvax._.extend({}, allConstructorProps, moduleProps); //如果原型上面是有type 属性的，那么说明，自己是type分类路由的一个分支，放到typeMap下面
        ...
        if (allConstructorProps.type) {
          ...
          _canvax._.extend(allProps[n].propertys, moduleProps);
        }
