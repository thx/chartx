...
function () {
  ...
  Tween.prototype._setupProperties = function (_object, _valuesStart, _valuesEnd, _valuesStartRepeat) {
    for (var property in _valuesEnd) {
      ...
      var propType = startValueIsArray ? 'array' : (0, _typeof3["default"])(startValue);
  ...
  Tween.prototype._updateProperties = function (_object, _valuesStart, _valuesEnd, value) {
    for (var property in _valuesEnd) {
      ...
      } else if ((0, _typeof3["default"])(end) === 'object' && end) {
        // eslint-disable-next-line
        // @ts-ignore FIXME?
...
function enabledAnimationFrame() {
  if (!_requestAid) {
    _requestAid = requestAnimationFrame(function () {
      ...
      _exports.update(); //tween自己会做length判断
      //};
...
function registTween(options) {
  ...
  if (opt.from && opt.to) {
    ...
    tween = new _exports.Tween(opt.from).to(opt.to, opt.duration).onStart(function () {
      ...
      opt.onStop(opt.from);
    }).repeat(opt.repeat).delay(opt.delay).easing(_exports.Easing[opt.easing.split(".")[0]][opt.easing.split(".")[1]]);
    tween.id = tid;
    tween.start();
    animate();
  }
