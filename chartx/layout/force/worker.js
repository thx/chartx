...
importScripts('physics/atoms.js');
importScripts('physics/barnes-hut.js');  
importScripts('physics/physics.js');  

var _ = {
    each:function(obj, callback){
        if (_.isArray(obj)){
            for (var i=0, j=obj.length; i<j; i++) callback( obj[i] , i )
        } else {
            for (var k in obj) callback(obj[k], k )
        }
    },
    map:function(arr, fn){
        var out = [];
